(function($, Waveform, Ciseaux){
  ...
  User.prototype.forkRepo = function(done) {
  	var self = this;
	  var mainRepoName = "beatproducer-projects";
		
		this.fork = this.github.getRepo(username, mainRepoName);
		this.repo = this.github.getRepo("s-a", mainRepoName);
		this.fork.show(function(err, repo) {
			if (err){
				self.repo.fork(function(err) {
					if (err){
						console.error(err);
						alert(mainRepoName + "\n\n" + err.request.responseText);
					} else {
						self.fork = self.github.getRepo(username, mainRepoName);
						if (done){
							done();
						}
					}
  ...
  User.prototype.newSong = function(songname, done) {
  	...
  	if (!self.fork){
  		this.fork = this.github.getRepo(username, "beatproducer-projects");
  	}
		self.fork.branch("master", songname, function(err) {
			if (err){
				if (err.error === 422){
					self.branch = songname;
					if (done){
						done();
					}
				} else {
  				alert("error creating song branch;\n" + err.request.responseText);
					console.log(err);
				}
			} else {
				self.branch = songname;
  ...
  User.prototype.saveSong = function(data, commitMessage, done) {
  		user.fork.write(user.branch, user.username + '/' + user.branch + '/' + user.branch + "." + new Date().getTime(), JSON.stringify(data), commitMessage, function(err) {
  			if (err){
  ...
  user.login(username, password, function(){
  	user.newSong("test", function() {
  		user.saveSong({foo: "bar2"}, 'changed the song 23', function() {
