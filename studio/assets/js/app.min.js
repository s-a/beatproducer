...
(function() {
    var Octokit, Promise, XMLHttpRequest, allPromises, createGlobalAndAMD, encode, err, injector, makeOctokit, newPromise, _, _ref, _this = this, __hasProp = {}.hasOwnProperty, __extends = function(child, parent) {
        function ctor() {
    ...
    _ = {}, _.isEmpty = function(object) {
        return 0 === Object.keys(object).length;
    }, _.isArray = Array.isArray || function(obj) {
        return "[object Array]" === toString.call(obj);
    }, _.defaults = function(object, values) {
        var key, _i, _len, _ref, _results;
        for (_ref = Object.keys(values), _results = [], _i = 0, _len = _ref.length; _len > _i; _i++) key = _ref[_i], 
        _results.push(function(key) {
            return null != object[key] ? object[key] : object[key] = values[key];
        }(key));
        return _results;
    }, _.each = function(object, fn) {
        var arr, key, _i, _len, _ref, _results;
        if (object) {
            for (_.isArray(object) && object.forEach(fn), arr = [], _ref = Object.keys(object), 
            _results = [], _i = 0, _len = _ref.length; _len > _i; _i++) key = _ref[_i], _results.push(function(key) {
        ...
        return ajax = function(options) {
            return newPromise(function(resolve, reject) {
                var name, value, xhr, _ref;
                xhr = new XMLHttpRequest(), xhr.dataType = options.dataType, "function" == typeof xhr.overrideMimeType && xhr.overrideMimeType(options.mimeType), 
                xhr.open(options.type, options.url), options.data && "GET" !== options.type && xhr.setRequestHeader("Content-Type", options.contentType), 
                _ref = options.headers;
                for (name in _ref) value = _ref[name], xhr.setRequestHeader(name, value);
                return xhr.onreadystatechange = function() {
                    var _name, _ref1;
                    return 4 === xhr.readyState ? (null != (_ref1 = options.statusCode) && "function" == typeof _ref1[_name = xhr.status] && _ref1[_name](), 
                    xhr.status >= 200 && xhr.status < 300 || 304 === xhr.status ? resolve(xhr) : reject(xhr)) : void 0;
            ...
            function Octokit(clientOptions) {
                ...
                null == clientOptions && (clientOptions = {}), _.defaults(clientOptions, {
                    rootURL: "https://api.github.com",
                    useETags: !0,
                    usePostInsteadOfPatch: !1
                }), _client = this, _listeners = [], ETagResponse = function() {
                    function ETagResponse(eTag, data, status) {
                        this.eTag = eTag, this.data = data, this.status = status;
                    }
                    return ETagResponse;
                }(), _cachedETags = {}, notifyStart = function(promise, path) {
                    return "function" == typeof promise.notify ? promise.notify({
                        type: "start",
                        path: path
                    }) : void 0;
                    ...
                    return null == options && (options = {
                        ...
                        isBase64: !1,
                        isBoolean: !1
                    }), "PATCH" === method && clientOptions.usePostInsteadOfPatch && (method = "POST"), 
                    /^http/.test(path) || (path = "" + clientOptions.rootURL + path), mimeType = void 0, 
                    options.isBase64 && (mimeType = "text/plain; charset=x-user-defined"), headers = {
                        Accept: "application/vnd.github.raw"
                    }, userAgent && (headers["User-Agent"] = userAgent), path in _cachedETags ? headers["If-None-Match"] = _cachedETags[path].eTag : headers["If-Modified-Since"] = "Thu, 01 Jan 1970 00:00:00 GMT", 
                    (clientOptions.token || clientOptions.username && clientOptions.password) && (auth = clientOptions.token ? "token " + clientOptions.token : "Basic " + base64encode("" + clientOptions.username + ":" + clientOptions.password), 
                    headers.Authorization = auth), promise = newPromise(function(resolve, reject) {
                        var ajaxConfig, always, onError, xhrPromise, _this = this;
                        return ajaxConfig = {
                            url: path,
                            type: method,
                            ...
                            dataType: options.raw ? void 0 : "json"
                        }, options.isBoolean && (ajaxConfig.statusCode = {
                            204: function() {
                                return resolve(!0);
                            },
                            404: function() {
                        ...
                        }), xhrPromise = ajax(ajaxConfig), always = function(jqXHR) {
                            var listener, rateLimit, rateLimitRemaining, _i, _len, _results;
                            for (notifyEnd(_this, path), rateLimit = parseFloat(jqXHR.getResponseHeader("X-RateLimit-Limit")), 
                            rateLimitRemaining = parseFloat(jqXHR.getResponseHeader("X-RateLimit-Remaining")), 
                            _results = [], _i = 0, _len = _listeners.length; _len > _i; _i++) listener = _listeners[_i], 
                            ...
                            if (always(jqXHR), 304 === jqXHR.status) return clientOptions.useETags && _cachedETags[path] ? (eTagResponse = _cachedETags[path], 
                            resolve(eTagResponse.data, eTagResponse.status, jqXHR)) : resolve(jqXHR.responseText, status, jqXHR);
                            if (204 === jqXHR.status && options.isBoolean) return resolve(!0, status, jqXHR);
                            if (jqXHR.responseText && "json" === ajaxConfig.dataType ? (data = JSON.parse(jqXHR.responseText), 
                            valOptions = {}, links = jqXHR.getResponseHeader("Link"), _.each(null != links ? links.split(",") : void 0, function(part) {
                                var discard, href, rel, _ref;
                                return _ref = part.match(/<([^>]+)>;\ rel="([^"]+)"/), discard = _ref[0], href = _ref[1], 
                                rel = _ref[2], valOptions["" + rel + "Page"] = function() {
                                    return _request("GET", href, null, options);
                                };
                            }), _.extend(data, valOptions)) : data = jqXHR.responseText, "GET" === method && options.isBase64) {
                                for (converted = "", i = _i = 0, _ref = data.length; _ref >= 0 ? _ref >= _i : _i >= _ref; i = _ref >= 0 ? ++_i : --_i) converted += String.fromCharCode(255 & data.charCodeAt(i));
                                data = converted;
                            }
                            return "GET" === method && jqXHR.getResponseHeader("ETag") && clientOptions.useETags && (eTag = jqXHR.getResponseHeader("ETag"), 
                            _cachedETags[path] = new ETagResponse(eTag, data, jqXHR.status)), resolve(data, jqXHR.status, jqXHR);
                        }), onError = function(jqXHR) {
                            var err, json;
                            return always(jqXHR), options.isBoolean && 404 === jqXHR.status ? resolve(!1) : "application/json; charset=utf-8" !== jqXHR.getResponseHeader("Content-Type") ? (err = new Error(jqXHR.responseText), 
                            err.status = jqXHR.status, err.__jqXHR = jqXHR, reject(err)) : (err = new Error("Github error: " + jqXHR.responseText), 
                            json = jqXHR.responseText ? JSON.parse(jqXHR.responseText) : "", err.error = json, 
                            err.status = jqXHR.status, err.__jqXHR = jqXHR, reject(err));
                        }, ("function" == typeof xhrPromise["catch"] ? xhrPromise["catch"](onError) : void 0) || xhrPromise.fail(onError);
                    }), notifyStart(promise, path), promise;
                }, toQueryString = function(options) {
                    var params;
                    return _.isEmpty(options) ? "" : (params = [], _.each(_.pairs(options), function(_arg) {
                        var key, value;
                        return key = _arg[0], value = _arg[1], params.push("" + key + "=" + encodeURIComponent(value));
                    }), "?" + params.join("&"));
                    ...
                    if (null === cachedETags || "object" != typeof cachedETags) throw new Error('BUG: argument of method "setCache" should be an object');
                    ...
                    var options;
                    return null == since && (since = null), options = {}, since && (options.since = since), 
                    _request("GET", "/users", options);
                }, this.getOrgRepos = function(orgName, type) {
                    return null == type && (type = "all"), _request("GET", "/orgs/" + orgName + "/repos?type=" + type + "&per_page=1000&sort=updated&direction=desc", null);
                }, this.getPublicGists = function(since) {
                    var getDate, options;
                    return null == since && (since = null), options = null, getDate = function(time) {
                        return Date === time.constructor ? time.toISOString() : time;
                    }, since && (options = {
                        since: getDate(since)
                    ...
                    return null == options && (options = {}), getDate = function(time) {
                        return Date === time.constructor ? time.toISOString() : time;
                    }, options.since && (options.since = getDate(options.since)), queryString = toQueryString(options), 
                    _request("GET", "/notifications" + queryString, null);
                }, User = function() {
                    function User(_username) {
                        var _cachedInfo, _rootPath;
                        null == _username && (_username = null), _rootPath = _username ? "/users/" + _username : "/user", 
                        _cachedInfo = null, this.getInfo = function(force) {
                            return null == force && (force = !1), force && (_cachedInfo = null), _cachedInfo ? resolvedPromise(_cachedInfo) : _request("GET", "" + _rootPath, null).then(function(info) {
                                return _cachedInfo = info;
                            });
                        }, this.getRepos = function(type, sort, direction) {
                            return null == type && (type = "all"), null == sort && (sort = "pushed"), null == direction && (direction = "desc"), 
                            ...
                            var isPublic;
                            if (!_username) throw new Error("BUG: This does not work for authenticated users yet!");
                            return isPublic = "", onlyPublic && (isPublic = "/public"), _request("GET", "/users/" + _username + "/received_events" + isPublic, null);
                        }, this.getEvents = function(onlyPublic) {
                            var isPublic;
                            if (!_username) throw new Error("BUG: This does not work for authenticated users yet!");
                            return isPublic = "", onlyPublic && (isPublic = "/public"), _request("GET", "/users/" + _username + "/events" + isPublic, null);
                        };
                    }
                    return User;
                }(), AuthenticatedUser = function(_super) {
                    function AuthenticatedUser() {
                        AuthenticatedUser.__super__.constructor.call(this), this.updateInfo = function(options) {
                            ...
                            return _.isArray(emails) || (emails = [ emails ]), _request("POST", "/user/emails", emails);
                        }, this.addEmail = function(emails) {
                            return _.isArray(emails) || (emails = [ emails ]), _request("DELETE", "/user/emails", emails);
                            ...
                            return null == options && (options = {}), options.name = name, _request("POST", "/user/repos", options);
                        }, this.getReceivedEvents = function(username, page) {
                            var currentPage;
                            return null == page && (page = 1), currentPage = "?page=" + page, _request("GET", "/users/" + username + "/received_events" + currentPage, null);
                    ...
                    return __extends(AuthenticatedUser, _super), AuthenticatedUser;
                }(User), Team = function() {
                    function Team(id) {
                        this.id = id, this.getInfo = function() {
                    ...
                    function Organization(name) {
                        this.name = name, this.getInfo = function() {
                            ...
                            var options;
                            return null == repoNames && (repoNames = null), null == permission && (permission = "pull"), 
                            options = {
                                name: name,
                                permission: permission
                            }, repoNames && (options.repo_names = repoNames), _request("POST", "/orgs/" + this.name + "/teams", options);
                        }, this.getMembers = function() {
                            ...
                            return null == options && (options = {}), options.name = name, _request("POST", "/orgs/" + this.name + "/repos", options);
                        }, this.getRepos = function() {
                    ...
                    function GitRepo(repoUser, repoName) {
                        var _repoPath;
                        this.repoUser = repoUser, this.repoName = repoName, _repoPath = "/repos/" + this.repoUser + "/" + this.repoName, 
                        this.deleteRepo = function() {
                            ...
                            return "" === path ? this.getRef("heads/" + branch) : this.getTree(branch, {
                                recursive: !0
                            }).then(function(tree) {
                                var file;
                                return file = _.select(tree, function(file) {
                                    return file.path === path;
                                })[0], (null != file ? file.sha : void 0) ? null != file ? file.sha : void 0 : rejectedPromise({
                                    message: "SHA_NOT_FOUND"
                                });
                            });
                        }, this.getContents = function(path, sha) {
                            var queryString;
                            return null == sha && (sha = null), queryString = "", null !== sha && (queryString = toQueryString({
                                ref: sha
                            })), _request("GET", "" + _repoPath + "/contents/" + path + queryString, null, {
                                raw: !0
                            ...
                            var queryString;
                            return null == options && (options = null), queryString = toQueryString(options), 
                            _request("GET", "" + _repoPath + "/git/trees/" + tree + queryString, null).then(function(res) {
                                return res.tree;
                            });
                        }, this.postBlob = function(content, isBase64) {
                            return "string" == typeof content && (isBase64 && (content = base64encode(content)), 
                            content = {
                                content: content,
                                encoding: "utf-8"
                            }), isBase64 && (content.encoding = "base64"), _request("POST", "" + _repoPath + "/git/blobs", content).then(function(res) {
                                return res.sha;
                            });
                            ...
                            return data = {
                                base_tree: baseTree,
                                tree: newTree
                            }, _request("POST", "" + _repoPath + "/git/trees", data).then(function(res) {
                                return res.sha;
                            });
                            ...
                            var data;
                            return _.isArray(parents) || (parents = [ parents ]), data = {
                                message: message,
                                parents: parents,
                                tree: tree
                            }, _request("POST", "" + _repoPath + "/git/commits", data).then(function(commit) {
                                return commit.sha;
                            });
                        }, this.updateHead = function(head, commit, force) {
                            var options;
                            return null == force && (force = !1), options = {
                                sha: commit
                            }, force && (options.force = !0), _request("PATCH", "" + _repoPath + "/git/refs/heads/" + head, options);
                        }, this.getCommit = function(sha) {
                            return _request("GET", "" + _repoPath + "/commits/" + sha, null);
                        }, this.getCommits = function(options) {
                            var getDate, queryString;
                            return null == options && (options = {}), options = _.extend({}, options), getDate = function(time) {
                                return Date === time.constructor ? time.toISOString() : time;
                            }, options.since && (options.since = getDate(options.since)), options.until && (options.until = getDate(options.until)), 
                            queryString = toQueryString(options), _request("GET", "" + _repoPath + "/commits" + queryString, null);
                        };
                    }
                    return GitRepo;
                }(), Branch = function() {
                    function Branch(git, getRef) {
                        var _getRef, _git;
                        _git = git, _getRef = getRef || function() {
                            throw new Error("BUG: No way to fetch branch ref!");
                        }, this.getCommit = function(sha) {
                            return _git.getCommit(sha);
                        }, this.getCommits = function(options) {
                            return null == options && (options = {}), options = _.extend({}, options), _getRef().then(function(branch) {
                                return options.sha = branch, _git.getCommits(options);
                            });
                            ...
                            return null == message && (message = "Removed " + path), null == sha && (sha = null), 
                            _getRef().then(function(branch) {
                                return sha ? _git.removeFile(path, message, sha, branch) : _git.getSha(branch, path).then(function(sha) {
                                    return _git.removeFile(path, message, sha, branch);
                                });
                            });
                        }, this.move = function(path, newPath, message) {
                            return null == message && (message = "Moved " + path), _getRef().then(function(branch) {
                                return _git._updateTree(branch).then(function(latestCommit) {
                                    return _git.getTree(latestCommit, {
                                        recursive: !0
                                    }).then(function(tree) {
                                        return _.each(tree, function(ref) {
                                            return ref.path === path && (ref.path = newPath), "tree" === ref.type ? delete ref.sha : void 0;
                                        }), _git.postTree(tree).then(function(rootTree) {
                            ...
                            var contents;
                            return null == message && (message = "Changed " + path), null == parentCommitSha && (parentCommitSha = null), 
                            contents = {}, contents[path] = {
                                content: content,
                                isBase64: isBase64
                            }, this.writeMany(contents, message, parentCommitSha);
                        }, this.writeMany = function(contents, message, parentCommitShas) {
                            return null == message && (message = "Changed Multiple"), null == parentCommitShas && (parentCommitShas = null), 
                            _getRef().then(function(branch) {
                                var afterParentCommitShas;
                                return afterParentCommitShas = function(parentCommitShas) {
                                    var promises;
                                    return promises = _.map(_.pairs(contents), function(_arg) {
                                        var content, data, isBase64, path;
                                        return path = _arg[0], data = _arg[1], content = data.content || data, isBase64 = data.isBase64 || !1, 
                                        _git.postBlob(content, isBase64).then(function(blob) {
                                            return {
                                                path: path,
                                                mode: "100644",
                                                type: "blob",
                                    ...
                                    }), allPromises(promises).then(function(newTrees) {
                                ...
                                }, parentCommitShas ? afterParentCommitShas(parentCommitShas) : _git._updateTree(branch).then(afterParentCommitShas);
                            });
                        };
                    }
                    return Branch;
                    ...
                    function Repository(options) {
                        var _repo, _user;
                        this.options = options, _user = this.options.user, _repo = this.options.name, this.git = new GitRepo(_user, _repo), 
                        this.repoPath = "/repos/" + _user + "/" + _repo, this.currentTree = {
                            branch: null,
                            sha: null
                        }, this.updateInfo = function(options) {
                            ...
                            var getRef;
                            return null == branchName && (branchName = null), branchName ? (getRef = function() {
                                return resolvedPromise(branchName);
                            }, new Branch(this.git, getRef)) : this.getDefaultBranch();
                        }, this.getDefaultBranch = function() {
                            var getRef, _this = this;
                            return getRef = function() {
                                return _this.getInfo().then(function(info) {
                                    return info.default_branch;
                                });
                            }, new Branch(this.git, getRef);
                            ...
                            return organization ? _request("POST", "" + this.repoPath + "/forks", {
                                organization: organization
                            }) : _request("POST", "" + this.repoPath + "/forks", null);
                            ...
                            _request("GET", "" + this.repoPath + "/notifications" + queryString, null);
                        }, this.getCollaborators = function() {
                            return _request("GET", "" + this.repoPath + "/collaborators", null);
                        }, this.addCollaborator = function(username) {
                            if (!username) throw new Error("BUG: username is required");
                            ...
                            if (null == username && (username = null), !username) throw new Error("BUG: username is required");
                            return _request("GET", "" + this.repoPath + "/collaborators/" + username, null, {
                                isBoolean: !0
                            });
                        }, this.canCollaborate = function() {
                            var _this = this;
                            return clientOptions.password || clientOptions.token ? _client.getLogin().then(function(login) {
                                return login ? _this.isCollaborator(login) : !1;
                            }).then(null, function() {
                                return !1;
                            }) : resolvedPromise(!1);
                            ...
                            var data;
                            return null == events && (events = [ "push" ]), null == active && (active = !0), 
                            data = {
                                name: name,
                                config: config,
                                events: events,
                                active: active
                            }, _request("POST", "" + this.repoPath + "/hooks", data);
                        }, this.editHook = function(id, config, events, addEvents, removeEvents, active) {
                            var data;
                            return null == config && (config = null), null == events && (events = null), null == addEvents && (addEvents = null), 
                            null == removeEvents && (removeEvents = null), null == active && (active = null), 
                            data = {}, null !== config && (data.config = config), null !== events && (data.events = events), 
                            null !== addEvents && (data.add_events = addEvents), null !== removeEvents && (data.remove_events = removeEvents), 
                            null !== active && (data.active = active), _request("PATCH", "" + this.repoPath + "/hooks/" + id, data);
                    ...
                    function Gist(options) {
                        var id, _gistPath;
                        this.options = options, id = this.options.id, _gistPath = "/gists/" + id, this.read = function() {
                            return _request("GET", _gistPath, null);
                        }, this.create = function(files, isPublic, description) {
                            return null == isPublic && (isPublic = !1), null == description && (description = null), 
                            options = {
                                isPublic: isPublic,
                                files: files
                            }, null != description && (options.description = description), _request("POST", "/gists", options);
                        }, this["delete"] = function() {
                            return _request("DELETE", _gistPath, null);
                        }, this.fork = function() {
                            return _request("POST", "" + _gistPath + "/forks", null);
                        }, this.update = function(files, description) {
                            return null == description && (description = null), options = {
                                files: files
                            }, null != description && (options.description = description), _request("PATCH", _gistPath, options);
                        }, this.star = function() {
                    ...
                    if (!user) throw new Error("BUG! user argument is required");
                    if (!repo) throw new Error("BUG! repo argument is required");
                    ...
                    return null == login && (login = null), login ? new User(login) : clientOptions.password || clientOptions.token ? new AuthenticatedUser() : null;
                }, this.getGist = function(id) {
                    return new Gist({
                        id: id
                    });
                }, this.getLogin = function() {
                    return clientOptions.password || clientOptions.token ? new User().getInfo().then(function(info) {
                        return info.login;
    ...
    }, "undefined" != typeof exports && null !== exports ? (Promise = this.Promise || require("es6-promise").Promise, 
    ...
    newPromise = function(fn) {
        ...
        var buffer;
        return buffer = new Buffer(str, "binary"), buffer.toString("base64");
    ...
    exports["new"] = function(options) {
        ...
        return null != _this.define ? _this.define("octokit", [], function() {
        ...
        _this.Octokit = Octokit, _this.Github = Octokit);
    }, this.Q ? (newPromise = function(fn) {
        var deferred, reject, resolve;
        return deferred = _this.Q.defer(), resolve = function(val) {
            return deferred.resolve(val);
        }, reject = function(err) {
            return deferred.reject(err);
        }, fn(resolve, reject), deferred.promise;
    }, allPromises = function(promises) {
        return this.Q.all(promises);
    }, createGlobalAndAMD(newPromise, allPromises)) : this.angular ? (injector = angular.injector([ "ng" ]), 
    injector.invoke(function($q) {
        return newPromise = function(fn) {
            var deferred, reject, resolve;
            return deferred = $q.defer(), resolve = function(val) {
                return deferred.resolve(val);
            }, reject = function(err) {
                return deferred.reject(err);
            }, fn(resolve, reject), deferred.promise;
    ...
    })) : (null != (_ref = this.jQuery) ? _ref.Deferred : void 0) ? (newPromise = function(fn) {
        var promise, reject, resolve;
        return promise = _this.jQuery.Deferred(), resolve = function(val) {
            return promise.resolve(val);
        }, reject = function(val) {
            return promise.reject(val);
        }, fn(resolve, reject), promise.promise();
    }, allPromises = function(promises) {
        var _ref1;
        return (_ref1 = _this.jQuery).when.apply(_ref1, promises).then(function() {
            var promises;
            return promises = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
        });
    }, createGlobalAndAMD(newPromise, allPromises)) : this.Promise ? (newPromise = function(fn) {
        return new _this.Promise(function(resolve) {
            return resolve.fulfill ? fn(resolve.resolve.bind(resolve), resolve.reject.bind(resolve)) : fn.apply(null, arguments);
        });
    }, allPromises = function(promises) {
        return _this.Promise.all.call(_this.Promise, promises);
    }, createGlobalAndAMD(newPromise, allPromises)) : (err = function(msg) {
        throw "undefined" != typeof console && null !== console && "function" == typeof console.error && console.error(msg), 
        new Error(msg);
    })("A Promise API was not found. Supported libraries that have Promises are jQuery, angularjs, and https://github.com/jakearchibald/es6-promise"));
}).call(this), function() {
    var JSONP, Waveform, __bind = function(fn, me) {
        return function() {
    ...
    window.Waveform = Waveform = function() {
        function Waveform(options) {
            if (this.redraw = __bind(this.redraw, this), this.container = options.container, 
            this.canvas = options.canvas, this.data = options.data || [], this.outerColor = options.outerColor || "transparent", 
            this.innerColor = options.innerColor || "#000000", this.interpolate = !0, options.interpolate === !1 && (this.interpolate = !1), 
            null == this.canvas) {
            ...
            this.patchCanvasForIE(this.canvas), this.context = this.canvas.getContext("2d"), 
            this.width = parseInt(this.context.canvas.width, 10), this.height = parseInt(this.context.canvas.height, 10), 
            options.data && this.update(options);
        }
        return Waveform.name = "Waveform", Waveform.prototype.setData = function(data) {
            ...
            return null != options.interpolate && (this.interpolate = options.interpolate), 
            this.interpolate === !1 ? this.setDataCropped(options.data) : this.setDataInterpolated(options.data), 
            this.redraw();
        }, Waveform.prototype.redraw = function() {
            var d, i, middle, t, _i, _len, _ref, _results;
            for (this.clear(), this.context.fillStyle = "function" == typeof this.innerColor ? this.innerColor() : this.innerColor, 
            middle = this.height / 2, i = 0, _ref = this.data, _results = [], _i = 0, _len = _ref.length; _len > _i; _i++) d = _ref[_i], 
            t = this.width / this.data.length, "function" == typeof this.innerColor && (this.context.fillStyle = this.innerColor(i / this.width, d)), 
            this.context.clearRect(t * i, middle - middle * d, t, middle * d * 2), this.context.fillRect(t * i, middle - middle * d, t, middle * d * 2), 
            _results.push(i++);
            return _results;
        }, Waveform.prototype.clear = function() {
            return this.context.fillStyle = this.outerColor, this.context.clearRect(0, 0, this.width, this.height), 
            this.context.fillRect(0, 0, this.width, this.height);
        }, Waveform.prototype.patchCanvasForIE = function(canvas) {
            var oldGetContext;
            return "undefined" != typeof window.G_vmlCanvasManager ? (canvas = window.G_vmlCanvasManager.initElement(canvas), 
            oldGetContext = canvas.getContext, canvas.getContext = function() {
                var ctx;
                return ctx = oldGetContext.apply(canvas, arguments), canvas.getContext = oldGetContext, 
                ctx;
            }) : void 0;
        }, Waveform.prototype.createCanvas = function(container, width, height) {
            var canvas;
            return canvas = document.createElement("canvas"), container.appendChild(canvas), 
            ...
            if (null == defaultValue && (defaultValue = 0), newData = [], data.length > limit) newData = data.slice(data.length - limit, data.length); else for (i = _i = 0, 
            _ref = limit - 1; _ref >= 0 ? _ref >= _i : _i >= _ref; i = _ref >= 0 ? ++_i : --_i) newData[i] = data[i] || defaultValue;
            return newData;
            ...
            for (newData = new Array(), springFactor = new Number((data.length - 1) / (fitCount - 1)), 
            newData[0] = data[0], i = 1; fitCount - 1 > i; ) tmp = i * springFactor, before = new Number(Math.floor(tmp)).toFixed(), 
            after = new Number(Math.ceil(tmp)).toFixed(), atPoint = tmp - before, newData[i] = this.linearInterpolate(data[before], data[after], atPoint), 
            i++;
            ...
            return null == options && (options = {}), innerColorWasSet = !1, that = this, {
                whileplaying: this.redraw,
                whileloading: function() {
                    var stream;
                    return innerColorWasSet || (stream = this, that.innerColor = function(x) {
                        return x < stream.position / stream.durationEstimate ? options.playedColor || "rgba(255,  102, 0, 0.8)" : x < stream.bytesLoaded / stream.bytesTotal ? options.loadedColor || "rgba(0, 0, 0, 0.8)" : options.defaultColor || "rgba(0, 0, 0, 0.4)";
                    }, innerColorWasSet = !0), this.redraw;
                }
            };
        ...
        return load = function(url) {
            var done, head, script;
            return script = document.createElement("script"), done = !1, script.src = url, script.async = !0, 
            script.onload = script.onreadystatechange = function() {
                return done || this.readyState && "loaded" !== this.readyState && "complete" !== this.readyState || (done = !0, 
                script.onload = script.onreadystatechange = null, !script || !script.parentNode) ? void 0 : script.parentNode.removeChild(script);
            ...
            query = -1 === (url || "").indexOf("?") ? "?" : "&", callbackName = callbackName || config.callbackName || "callback", 
            uniqueName = callbackName + "_json" + ++counter, params = params || {};
            ...
            var config;
            return config = obj;
        }, counter = 0, head = void 0, query = void 0, key = void 0, window = this, config = {}, 
        {
            get: jsonp,
            init: setDefaults
        };
    }();
}.call(this), function($, Waveform, Ciseaux) {
    function toFixed(value, count) {
        var precision = count || 0, power = Math.pow(10, precision), absValue = Math.abs(Math.round(value * power)), result = (0 > value ? "-" : "") + String(Math.floor(absValue / power));
        if (precision > 0) {
    ...
    var sampleDbPath = "./../audio/sample-db/", AudioContext = window.AudioContext || window.webkitAudioContext, onPlayBackIntervalInMilliseconds = 100;
    /* ******************** HELPER FUNCTIONS ************************************************************  */
    $.ajaxTransport("+binary", function(options, originalOptions, jqXHR) {
        // check for conditions and support for blob / arraybuffer response type
        // check for conditions and support for blob / arraybuffer response type
        return window.FormData && (options.dataType && "binary" === options.dataType || options.data && (window.ArrayBuffer && options.data instanceof ArrayBuffer || window.Blob && options.data instanceof Blob)) ? {
            // create new XMLHttpRequest
            send: function(headers, callback) {
                // setup all variables
                var xhr = new XMLHttpRequest(), url = options.url, type = options.type, async = options.async || !0, // blob or arraybuffer. Default is blob
                ...
                xhr.responseType = dataType, xhr.send(data);
            },
            abort: function() {
                jqXHR.abort();
            }
    ...
    var Speech = function() {
        return this.say = function(text, done) {
            if (window.SpeechSynthesisUtterance && window.speechSynthesis) {
                var msg = new SpeechSynthesisUtterance();
                //msg.voice = voices[1]; // Note: some voices don't support altering params
                msg.voiceURI = "native", msg.volume = 1, // 0 to 1
                // msg.rate = 0; // 0.1 to 10
                // msg.pitch = 1; //0 to 2
                msg.text = text, msg.lang = "en-US";
                var callbackDone = !1;
                msg.onend = function() {
                    callbackDone = !0, console.log("DONE!"), done && done();
                }, window.speechSynthesis.speak(msg), window.setTimeout(function() {
                    callbackDone || done && done();
                }, 2e3);
            } else done && done();
        }, this;
    }, Device = function(_el) {
        _el && (this._el = _el), this.audioContext = new AudioContext(), this.bufSrc = null, 
        //  7  this._chored = false;
        //    this._db = {};
        this._comp = this.audioContext.createDynamicsCompressor(), this._comp.connect(this.audioContext.destination), 
        this._comp.threshold.value = -18, this._comp.knee.value = 8, this.destination = this._comp, 
        this.transportTime = 0, this._loadingProgressIndicatorInterval = null, this.markers = [], 
        this._regions = [];
        var self = this;
    ...
    Device.prototype.addMarker = function(seconds) {
        ...
        if (seconds) {
            ...
            for (var i = 0; i < seconds.length; i++) {
                var second = seconds[i], pos = this.spectrumPosition({
                    seconds: second
                });
                this.markers.push(pos);
            }
            this.computeSpectrumPositions(), this.refreshSpectrum();
        }
    }, Device.prototype.onResize = function() {
        var self = this;
        window.clearTimeout(this.___to), this.___to = window.setTimeout(function() {
            self.computeSpectrumPositions(), self.initializeSpectrum();
        }, 400);
    ...
    }, Device.prototype.get = function(url, done) {
        {
            ...
            $.ajax({
                ...
                success: function(data) {
                    done && $.proxy(done, self)(data);
                }
            });
        }
    }, Device.prototype.load = function(url, done) {
        var self = this, renderData = function(audioBuffer) {
            self.tape = new Ciseaux.Tape(audioBuffer), self.renderAudio(function() {
                self.setLoadingProgressIndicator(!1), self.initializeSpectrum(), done && $.proxy(done, self)();
            });
        }, processData = function(arraybuffer) {
            self.audioContext.decodeAudioData(arraybuffer, function(decodedData) {
                //self.rawData = decodedData;
                renderData(decodedData);
            });
        };
        "string" == typeof url ? this.get(url, function(arraybuffer) {
        ...
        if (activate) {
            ...
            var canvas = this.$().find(".device-spectrum").get(0), data = [], waveform = new Waveform({
                container: canvas,
                interpolate: !1
            }), ctx = waveform.context, gradient = ctx.createLinearGradient(0, 0, 0, waveform.height);
            gradient.addColorStop(0, "#f60"), gradient.addColorStop(1, "#ff1b00"), waveform.innerColor = gradient;
            var i = 0;
            this._loadingProgressIndicatorInterval = setInterval(function() {
                i++, data.push(Math.cos(i / 25) - .2 + .3 * Math.random()), waveform.update({
                    data: data
                });
            }, 1);
        } else this.$().find("canvas").remove(), window.clearInterval(this._loadingProgressIndicatorInterval);
    }, Device.prototype.computeSpectrumPositions = function() {
        if (this.waveform) for (var durationX = $(this.waveform.container).width(), i = 0; i < this.markers.length; i++) {
            var marker = this.markers[i];
            if (0 !== marker.percent && !marker.percent) throw "Need percantage value to compute slice positions in spectrum.";
            marker.x = durationX / 100 * marker.percent;
        }
    }, Device.prototype.spectrumPosition = function(position) {
        var duration = this.audioBuffer.duration, result = {};
        if (position.x && (result.x = position.x, result.percent = result.x / $(this.waveform.container).width() * 100, 
        result.seconds = result.percent / 100 * duration), 0 === position.seconds && (result.x = 0, 
        result.percent = 0, result.seconds = 0), position.seconds) {
            var durationX = $(this.waveform.container).width();
            result.percent = position.seconds / duration * 100, result.x = durationX / 100 * result.percent, 
            result.seconds = position.seconds;
        }
        return result;
    }, Device.prototype.refreshSpectrum = function(drawn) {
        this.waveform && (drawn || this.waveform.redraw(), this.refreshSpectrumMarkers(), 
        this.spectrumPositionX && vLine(this.waveform.context, this.spectrumPositionX, "#f60"));
    };
    var vLine = function(context, x, color) {
        context.beginPath(), context.moveTo(x, 0), context.lineTo(x, 190), context.strokeStyle = color, 
        context.stroke();
    };
    Device.prototype.spectrumCursorPosition = function(x) {
        this.spectrumPositionX = x, this.refreshSpectrum();
    }, Device.prototype.refreshSpectrumMarkers = function() {
        for (var i = 0; i < this.markers.length; i++) {
            var marker = this.markers[i];
            vLine(this.waveform.context, marker.x, "#fff"), vLine(this.waveform.context, marker.x, "#fff");
        }
        ...
        if (canvas) {
            ...
            this.waveform = new Waveform({
                container: canvas,
                interpolate: !0,
                innerColor: function(i) {
                    var duration = this.device.audioBuffer.duration, currentTime = this.device.transportTime / 1e3, perc = currentTime / duration;
                    //console.log(i,d);
                    //console.log(i,d);
                    return perc > i ? "#f60" : "#676F98";
                }
            }), this.waveform.device = this, this.waveform.update({
                data: data
            });
            var self = this;
            self.refreshSpectrum(!0);
        }
    }, Device.prototype.renderAudio = function(done) {
        var self = this;
        this.tape.render(this.audioContext, 2).then(function(audioBuffer) {
            self.audioBuffer = audioBuffer, done && $.proxy(done, self)(audioBuffer);
        });
    }, Device.prototype.onPlayBack = function(reset) {
        reset || (this.transportTime += onPlayBackIntervalInMilliseconds), this.refreshSpectrum();
        var time = 0;
        this.transportTime > 0 && (time = this.transportTime / 1e3), this._el.setState({
            currentTime: toFixed(time, 1)
        });
    }, Device.prototype.play = function(done) {
        if (this.pause(), this.tape) {
            var self = this;
            self.bufSrc = self.audioContext.createBufferSource(), self.bufSrc.buffer = self.audioBuffer, 
            self.bufSrc.connect(self.destination), self.bufSrc.onended = function() {
                window.clearInterval(self.onPlayBackTimerInterval), $.proxy(done, self);
            };
            ...
            var offset = 0;
            this.transportTime > 0 && (offset = this.transportTime / 1e3), self.bufSrc.start(0, offset);
        }
    }, Device.prototype.stop = function() {
        var self = this;
        self.pause(), self.transportTime = 0, self.onPlayBack(!0);
    }, Device.prototype.pause = function() {
        this.tape && this.bufSrc && (window.clearInterval(this.onPlayBackTimerInterval), 
        this.bufSrc.stop());
    }, Device.prototype.open = function(url, done) {
        var self = this;
        self._el ? (self.load(url, function() {
            done && $.proxy(done, self)(self), self.chopSlices();
        }), self.setLoadingProgressIndicator(!0)) : self.render(function() {
            self._el = this, self.setLoadingProgressIndicator(!0), self.load(url, function() {
                done && $.proxy(done, self)(self), self.chopSlices();
            });
        });
        ...
        this.markers = this.markers.sort(sortNumber), this.slices = [];
        for (var i = 0; i < this.markers.length - 1; i++) {
            var start = this.markers[i].seconds, length = this.markers[i + 1].seconds - this.markers[i].seconds, tape = this.tape.slice(start, length);
            this.slices.push(new Slice(tape));
        }
    }, Device.prototype.config = function() {
        var self = this, slices = function() {
            for (var result = [], i = 0; i < self.markers.length; i++) result.push(self.markers[i].seconds);
            return result;
        }, patterns = function() {
            for (var result = [], i = 0; i < self.patterns.length; i++) {
    ...
    var Project = function(parentElement) {
        return this.parentElement = parentElement, this;
    };
    Project.prototype.$ = function() {
        return $(this._el.getDOMNode());
    }, Project.prototype.newDevice = function(deviceConfig, done) {
        var self = this, device = new Device(), childDevices = self._el.state.devices, onComplete = function() {
            device.addMarker(deviceConfig.slices), device.chopSlices(), done && $.proxy(done, self)(device);
        };
        // render device;
        if (device.project = self, childDevices.push(device), self._el.setState({
            devices: childDevices
        }), deviceConfig.sample) {
            var shortName = deviceConfig.sample.split("/");
            shortName.length > 1 && shortName.shift(), shortName = shortName.join("//"), device.name = shortName, 
            device.filename = deviceConfig.sample, device._el.setState({
                name: shortName
            }), device.open(sampleDbPath + deviceConfig.sample, onComplete);
        }
        device.patterns = [];
        for (var p = 0; p < deviceConfig.patterns.length; p++) device.patterns.push(new Pattern(device, deviceConfig.patterns[p]));
        deviceConfig.tape && (device.tape = deviceConfig.tape, device.load(null, onComplete));
        ...
        self.render(function() {
            self._el = this, done && $.proxy(done, self)();
        });
    }, Project.prototype.open = function(projectConfig, done) {
        var self = this;
        self.all || (self.all = projectConfig.devices.length), self._el.setState({
            bpm: projectConfig.bpm,
            name: projectConfig.name
        });
        for (var onOpened = function() {
            self.all--, 0 === self.all && (self.all = null, done(projectConfig));
        }, i = 0; i < projectConfig.devices.length; i++) self.newDevice(projectConfig.devices[i], onOpened);
    }, Project.prototype.config = function() {
        for (var result = {
            name: this.name,
            bpm: this.bpm,
            devices: []
        }, i = 0; i < this._el.state.devices.length; i++) result.devices.push(this._el.state.devices[i].config());
        return result;
    ...
    Project.prototype.$ = function() {
        return $(this._el.getDOMNode());
    }, Studio.prototype.Speech = Speech, Studio.prototype.Project = Project, Studio.prototype.renderGUI = function(parentElement, done) {
        var self = this;
        self.project = new self.Project(parentElement), self.project.studio = self, self.project.renderGUI(function() {
            self.patternEditor = new PatternEditor(), self.patternEditor.renderGUI(function() {
        ...
        $("#gui-loading-progress").fadeIn("fast", function() {}), //$(function() {
        $("body").one("react-components-ready", function() {
            window.gui.alert("loading project"), self.renderGUI(document.getElementById("content"), function() {
                initialProjectConfig ? (self.project.bpm = initialProjectConfig.bpm, self.project.name = initialProjectConfig.name, 
                self.project.open(initialProjectConfig, function(projectConfig) {
                    ...
                    done && $.proxy(done, self)(initialProjectConfig);
                })) : done && $.proxy(done, self)(initialProjectConfig);
            });
        });
    };
    /* ******************** SLICE ************************************************************  */
    var daAudioContext = new AudioContext(), Slice = function(tape) {
        var g = 1;
        //  7  this._chored = false;
        //    this._db = {};
        return tape = Ciseaux.concat(tape.split(1e3).map(function(tape, i) {
            var vol = .1;
            return i > 990 && (g -= vol), tape.gain(g);
        })), this.tape = tape, this.audioContext = daAudioContext, this.bufSrc = null, this._comp = this.audioContext.createDynamicsCompressor(), 
        this._comp.connect(this.audioContext.destination), this._comp.threshold.value = -18, 
        this._comp.knee.value = 8, this.destination = this._comp, this;
    };
    Slice.prototype.play = function(done) {
        if (this.tape) {
            var self = this;
            this.tape.render(this.audioContext, 2).then(function(audioBuffer) {
                self.audioBuffer = audioBuffer, self.bufSrc = self.audioContext.createBufferSource(), 
                self.bufSrc.buffer = self.audioBuffer, self.bufSrc.connect(self.destination), self.bufSrc.onended = function() {
                    done && $.proxy(done, self);
    ...
    var Pattern = function(device, patternConfig) {
        //  7  this._chored = false;
        //    this._db = {};
        return this.device = device, this.id = patternConfig.id, this.bars = patternConfig.bars, 
        this.sequence = patternConfig.sequence, this.patternConfig = patternConfig, this.audioContext = daAudioContext, 
        this.bufSrc = null, this._comp = this.audioContext.createDynamicsCompressor(), this._comp.connect(this.audioContext.destination), 
        this._comp.threshold.value = -18, this._comp.knee.value = 8, this.destination = this._comp, 
        this;
    };
    Pattern.prototype.toString = function(sliceIndex) {
        for (var sequence = this.sequence, result = "", i = 0; i < sequence.length; i++) {
            var subSequence = sequence[i];
            result += -1 === subSequence.indexOf(sliceIndex) ? " " : "a";
        }
        return result;
    }, Pattern.prototype.renderSequence = function(done) {
        for (var tape, bpm = this.device.project.bpm, numerOfBars = 1, durationPerStep = 6e4 / bpm / 1e3 / (2 * numerOfBars), i = 0; i < this.device.slices.length; i++) {
            var sliceTape = this.device.slices[i].tape, tape1 = new Ciseaux.Sequence(this.toString(i + 1), durationPerStep, {
                a: sliceTape
            }).apply();
            tape = tape ? tape.mix(tape1) : tape1;
        }
        this.tape = tape, done(tape);
    }, Pattern.prototype.play = function(done) {
        var self = this;
        self.renderSequence(function() {
            self.tape.render(self.audioContext, 2).then(function(audioBuffer) {
                self.audioBuffer = audioBuffer, self.bufSrc = self.audioContext.createBufferSource(), 
                self.bufSrc.buffer = self.audioBuffer, self.bufSrc.connect(self.destination), self.bufSrc.onended = function() {
                    $.proxy(done, self);
                }, self.bufSrc.loop = !0, self.bufSrc.start(0, 0);
            });
        });
    }, Pattern.prototype.stop = function() {
        var self = this;
        self.pause(), self.transportTime = 0;
    }, Pattern.prototype.pause = function() {
        this.tape && this.bufSrc && this.bufSrc.stop();
    };
    /* ******************** USER ************************************************************  */
    var User = function() {
        return this.mainRepoName = "beatproducer-projects", this;
    };
    User.prototype.login = function(username, password, done) {
        this.github = new window.Octokit({
            ...
            password: password,
            auth: "basic"
        }), this.username = username, this.initialzeRepo(done);
    }, User.prototype.forkRepo = function(done) {
        var self = this;
        this.fork = this.github.getRepo(this.username, self.mainRepoName), this.repo = this.github.getRepo("s-a", self.mainRepoName);
        var onRejected = function(res) {
            401 === res.status ? self.onError(res) : self.repo.fork().then(function() {
                window.setTimeout(function() {
                    // give the fork some time
                    self.fork = self.github.getRepo(self.username, self.mainRepoName), done && done();
                }, 5e3);
            }, self.onError);
        };
        ...
        $("#gui-loading-progress").fadeIn("fast", function() {
            self.branchname = songname, self.branch = self.fork.getBranch("master"), self.branch.createBranch(songname).then(function() {
                self.branch = self.fork.getBranch(songname), done();
            }, function() {
                self.branch = self.fork.getBranch(songname), done();
            });
        });
        ...
        this.branch.write(filename, data, commitMessage, !1).then(done, this.onError);
    }, User.prototype.onError = function(a) {
        var res = JSON.parse(a.message).errors;
        res ? res = res[res.length - 1] : (res = JSON.parse(a.message), res.resource = "login");
        var msg = res.resource + "\n\n" + res.message;
        $("#gui-loading-progress").fadeOut("fast", function() {
            "login" === res.resource && window.gui.onLoginClick(), window.gui.alert(msg);
        });
    ...
    /* ******************** GUI ************************************************************  */
    var GUI = function() {
        return AudioContext ? (this.credentials = {
            uid: $.cookie("uid"),
            pwd: $.cookie("pwd")
        }, $(".input-login-uid:first").val(this.credentials.uid), $(".input-login-pwd:first").val(this.credentials.pwd), 
        this.initEventListeners()) : (this.alert("Oh NO! Your Browser does not suppert AudioContext!?. We suggest Google Chrome to use this website. Maybe one day your browser will support new web technologies."), 
        window.setTimeout(function() {
    ...
    GUI.prototype.alert = function(msg) {
        ...
        $("body").append(toast), toast.get(0).addEventListener("core-overlay-close-completed", function() {
            toast.remove();
        }), toast.get(0).show && toast.get(0).show();
    }, GUI.prototype.onLoginClick = function() {
        document.querySelector("#login-dialog").toggle();
    }, GUI.prototype.onPublishClick = function() {
        var username = window.gui.credentials.uid, password = window.gui.credentials.pwd;
        if ("" === username || "" === password) window.gui.onLoginClick(); else {
            var user = new window.boom.User();
            user.login(username, password, function() {
                user.newSong("test", function() {
                    var msg = "changed the song " + new Date().getTime().toString(), data = studio.project.configString();
        ...
        chk.checked && ($.cookie("uid", window.gui.credentials.uid, {
            expires: 365
        }), $.cookie("pwd", window.gui.credentials.pwd, {
            expires: 365
        }));
        ...
        window.addEventListener("polymer-ready", function() {
            ...
            $("#studio-button-publish").click(self.onPublishClick), $("#studio-button-login").click(self.onLoginClick);
        });
    }, /* ******************** PUBLIC ************************************************************  */
...
var studio = new window.boom.Studio(), bpm = document.querySelector("#bpm");

bpm.addEventListener("core-change", function() {
    studio.project && (studio.project.bpm = bpm.value);
});
...
}, gui = window.gui = new window.boom.GUI(studio);

studio.init(defaultProject, function() {
    document.title = this.project.name, bpm.value = this.project.bpm, $("#gui-loading-progress").fadeOut("fast", function() {});
}), function($, boom, BoomDevice, BoomProject, BoomPatternEditor) {
    var sampleDbPath = "./../audio/sample-db/", Project = React.createClass({
        displayName: "Project",
        ...
        render: function() {
            ...
            return React.createElement("div", {
                className: "device-element-inset project-rack"
            }, devices);
        }
    }), Device = React.createClass({
        displayName: "Device",
        handleOnOpenPatternEditorClick: function() {
            $("#pattern-editor").slideDown("fast"), window.studio.patternEditor.open(this.state.device);
        },
        ...
        cursorPosition: function(evt) {
            var $el = $(evt.target), parentOffset = $el.parent().offset(), x = evt.pageX - parentOffset.left;
            return x;
        },
        onSprectrumMouseLeave: function() {
        ...
        onSpectrumMouseClick: function(evt) {
            var device = this.state.device, pos = device.spectrumPosition({
                x: this.cursorPosition(evt)
            });
            document.title = pos.seconds + " sec", device.markers.push(pos), device.refreshSpectrum(), 
            device.chopSlices(), device.project.studio.patternEditor.open(device);
        },
        getInitialState: function() {
        ...
        render: function() {
            return React.createElement("div", {
                className: "device-element-outset device"
            }, React.createElement("strong", {
                className: "device-name"
            }, this.state.name), " ", React.createElement("small", {
                className: "device-transport-time"
            }, this.state.currentTime), React.createElement("div", null, React.createElement("div", {
                onClick: this.onSpectrumMouseClick,
                onMouseMove: this.onSprectrumMouseMove,
                onMouseLeave: this.onSprectrumMouseLeave,
                className: "device-spectrum device-element-inset"
            })), React.createElement("a", {
                className: "device-button device-button-play",
                href: "javascript:void(0);",
                onClick: this.handleOnPlayClick
            }, React.createElement("img", {
                width: "32",
                src: "assets/images/ic_play_circle_fill_48px.svg",
                type: "image/svg+xml"
            })), React.createElement("a", {
                className: "device-button device-button-pause",
                href: "javascript:void(0);",
                onClick: this.handleOnPauseClick
            }, React.createElement("img", {
                width: "32",
                src: "assets/images/ic_pause_circle_fill_48px.svg",
                type: "image/svg+xml"
            })), React.createElement("a", {
                className: "device-button device-button-stop",
                href: "javascript:void(0);",
                onClick: this.handleOnStopClick
            }, React.createElement("img", {
                width: "32",
                src: "assets/images/ic_stop_48px.svg"
            })), React.createElement("a", {
                className: "device-button device-button-opensample",
                href: "javascript:void(0);",
                onClick: this.handleOnOpenClick
            }, React.createElement("img", {
                width: "32",
                src: "assets/images/ic_folder_open_48px.svg"
            })), React.createElement("a", {
                className: "device-button device-button-pattern-editor",
                href: "javascript:void(0);",
                onClick: this.handleOnOpenPatternEditorClick
            }, React.createElement("img", {
                width: "32",
                src: "assets/images/ic_queue_music_48px.svg"
            })));
        }
    }), PatternEditor = React.createClass({
        ...
        keyIsEquiped: function(index) {
            return this.state && this.state.connectedDevice && this.state.connectedDevice.slices[index] ? !0 : !1;
        },
        handleOnKeyClick: function(e) {
            sliceIndex = parseInt($(e.currentTarget).attr("data")), this.state.connectedDevice.slices[sliceIndex].play();
        },
        handleOnPlayClick: function() {
        ...
        handleOnStepClick: function(e) {
            var $el = $(e.target), sliceIndex = parseInt($el.data("slice-index")), step = parseInt($el.data("step")), device = this.state.connectedDevice, sequence = device.patterns[0].sequence, subSequence = sequence[step], sliceIdx = subSequence.indexOf(sliceIndex);
            -1 === sliceIdx ? subSequence.push(sliceIndex) : subSequence.splice(sliceIdx, 1), 
        ...
        render: function() {
            for (var self = this, baseNotes = [ "C", "C#", "D", "D#", "E", "F", "F#", "G", "G#", "A", "A#", "B" ], notes = [].concat(baseNotes).concat(baseNotes).concat(baseNotes).concat(baseNotes).concat(baseNotes).concat(baseNotes).concat(baseNotes).concat(baseNotes), octave = 4, index = 0, getCols = function(note) {
                index++;
                var cols = [], keyClassName = "pattern-keyboard-key";
                if (-1 !== note.indexOf("#") && (keyClassName += " pattern-keyboard-key-sharp"), 
                "B" === note && octave--, self.keyIsEquiped(index - 1)) {
                    cols.push(React.createElement("div", {
                        data: index - 1,
                        onClick: self.handleOnKeyClick,
                        className: keyClassName
                    }, note, octave)), -1 !== note.indexOf("#") && cols.push(React.createElement("div", {
                        onClick: self.handleOnKeyClick,
                        className: "pattern-keyboard-step-offset"
                    }, React.createElement("div", {
                        onClick: self.handleOnKeyClick,
                        className: "pattern-keyboard-step-offset-top"
                    }), React.createElement("div", {
                        className: "pattern-keyboard-step-offset-bottom"
                    })));
                    ...
                    if (self.state && self.state.connectedDevice) {
                        ...
                        if (pattern) for (var bar = 0; bar < pattern.bars; bar++) for (var steps = pattern.sequence.length, step = 0; steps > step; step++) {
                            var className = "pattern-keyboard-step", subSequence = pattern.sequence[step];
                            -1 !== subSequence.indexOf(index) && (className += " pattern-keyboard-step-active"), 
                            cols.push(React.createElement("div", {
                                onClick: self.handleOnStepClick,
                ...
                return cols;
            }, rows = [], i = notes.length - 1; i >= 0; i--) {
                var note = notes[i], rowClassName = "pattern-row";
                -1 !== note.indexOf("#") && (rowClassName += " pattern-row-sharp");
            ...
            return React.createElement("div", {
                ...
                className: "device-name"
            }, "PE"), " - ", React.createElement("small", {
                className: "device-transport-time"
            }, this.state.connectedDeviceName), " | ", React.createElement("small", {
                className: "device-transport-time"
            }, "PATTERN NAME"), React.createElement("div", {
                className: "device-element-outset pattern-editor"
            }, React.createElement("div", {
                className: "pattern"
            }, rows)));
        }
    ...
    BoomDevice.prototype.render = function(done) {
        var self = this;
        React.render(React.createElement(Device, null), self.parentElement, done);
    }, BoomProject.prototype.render = function(done) {
        var self = this;
        React.render(React.createElement(Project, null), self.parentElement, done);
    }, BoomPatternEditor.prototype.render = function(done) {
        React.render(React.createElement(PatternEditor, null), document.getElementById("pattern-editor"), done);
 ...
 */
// @version 0.5.5
window.WebComponents = window.WebComponents || {}, function(scope) {
    var flags = scope.flags || {}, file = "webcomponents.js", script = document.querySelector('script[src*="' + file + '"]');
    if (!flags.noOpts) {
        if (location.search.slice(1).split("&").forEach(function(o) {
            o = o.split("="), o[0] && (flags[o[0]] = o[1] || !0);
        }), script) for (var a, i = 0; a = script.attributes[i]; i++) "src" !== a.name && (flags[a.name] = a.value || !0);
        if (flags.log) {
            var parts = flags.log.split(",");
            flags.log = {}, parts.forEach(function(f) {
                flags.log[f] = !0;
            });
        } else flags.log = {};
    }
    flags.shadow = flags.shadow || flags.shadowdom || flags.polyfill, flags.shadow = "native" === flags.shadow ? !1 : flags.shadow || !HTMLElement.prototype.createShadowRoot, 
    flags.register && (window.CustomElements = window.CustomElements || {
        flags: {}
    }, window.CustomElements.flags.register = flags.register), scope.flags = flags;
...
}(), window.ShadowDOMPolyfill = {}, function(scope) {
    "use strict";
    function detectEval() {
    ...
    function mixin(to, from) {
        for (var names = getOwnPropertyNames(from), i = 0; i < names.length; i++) {
            var name = names[i];
    ...
    function defineNonEnumerableDataProperty(object, name, value) {
        nonEnumerableDataDescriptor.value = value, defineProperty(object, name, nonEnumerableDataDescriptor);
    }
    ...
    function installProperty(source, target, allowMethod) {
        for (var names = getOwnPropertyNames(source), i = 0; i < names.length; i++) {
            var name = names[i];
            if ("polymerBlackList_" !== name && !(name in target || source.polymerBlackList_ && source.polymerBlackList_[name])) {
                isFirefox && source.__lookupGetter__(name);
                var getter, setter, descriptor = getDescriptor(source, name);
                if (allowMethod && "function" == typeof descriptor.value) target[name] = getMethod(name); else {
                    var isEvent = isEventHandlerName(name);
                    getter = isEvent ? scope.getEventHandlerGetter(name) : getGetter(name), (descriptor.writable || descriptor.set || isBrokenSafari) && (setter = isEvent ? scope.getEventHandlerSetter(name) : getSetter(name));
    ...
    function registerInternal(nativePrototype, wrapperConstructor, opt_instance) {
        ...
        opt_instance && registerInstanceProperties(wrapperPrototype, opt_instance), defineNonEnumerableDataProperty(wrapperPrototype, "constructor", wrapperConstructor), 
    ...
    function createWrapperConstructor(superWrapperConstructor) {
        ...
        return p.constructor = GeneratedWrapper, GeneratedWrapper.prototype = p, GeneratedWrapper;
    }
    ...
    function wrap(impl) {
        return null === impl ? null : (assert(isNative(impl)), impl.__wrapper8e3dd93a60__ || (impl.__wrapper8e3dd93a60__ = new (getWrapperConstructor(impl))(impl)));
    }
    function unwrap(wrapper) {
        return null === wrapper ? null : (assert(isWrapper(wrapper)), wrapper.__impl4cf1e782hg__);
    }
    function unsafeUnwrap(wrapper) {
        return wrapper.__impl4cf1e782hg__;
    }
    function setWrapper(impl, wrapper) {
        wrapper.__impl4cf1e782hg__ = impl, impl.__wrapper8e3dd93a60__ = wrapper;
    }
    ...
    function defineGetter(constructor, name, getter) {
        getterDescriptor.get = getter, defineProperty(constructor.prototype, name, getterDescriptor);
    }
    ...
    var constructorTable = new WeakMap(), nativePrototypeTable = new WeakMap(), wrappers = Object.create(null), hasEval = detectEval(), defineProperty = Object.defineProperty, getOwnPropertyNames = Object.getOwnPropertyNames, getOwnPropertyDescriptor = Object.getOwnPropertyDescriptor, nonEnumerableDataDescriptor = {
        value: void 0,
        configurable: !0,
        enumerable: !1,
        writable: !0
    ...
    scope.isWrapper = isWrapper, scope.isWrapperFor = isWrapperFor, scope.mixin = mixin, 
    scope.nativePrototypeTable = nativePrototypeTable, scope.oneOf = oneOf, scope.registerObject = registerObject, 
    scope.registerWrapper = register, scope.rewrap = rewrap, scope.setWrapper = setWrapper, 
    scope.unsafeUnwrap = unsafeUnwrap, scope.unwrap = unwrap, scope.unwrapIfNeeded = unwrapIfNeeded, 
    ...
    ArraySplice.prototype = {
        calcEditDistances: function(current, currentStart, currentEnd, old, oldStart, oldEnd) {
            ...
            for (var i = 1; rowCount > i; i++) for (var j = 1; columnCount > j; j++) if (this.equals(current[currentStart + j - 1], old[oldStart + i - 1])) distances[i][j] = distances[i - 1][j - 1]; else {
        ...
        calcSplices: function(current, currentStart, currentEnd, old, oldStart, oldEnd) {
            ...
            for (var ops = this.spliceOperationsFromEditDistances(this.calcEditDistances(current, currentStart, currentEnd, old, oldStart, oldEnd)), splice = void 0, splices = [], index = currentStart, oldIndex = oldStart, i = 0; i < ops.length; i++) switch (ops[i]) {
              case EDIT_LEAVE:
        ...
        sharedPrefix: function(current, old, searchLength) {
            for (var i = 0; searchLength > i; i++) if (!this.equals(current[i], old[i])) return i;
            return searchLength;
        },
...
}(window.ShadowDOMPolyfill), function(context) {
    "use strict";
    function handle() {
        pending = !1;
    ...
    function setEndOfMicrotask(func) {
        callbacks.push(func), pending || (pending = !0, timerFunc(handle, 0));
    }
    var timerFunc, OriginalMutationObserver = window.MutationObserver, callbacks = [], pending = !1;
    if (OriginalMutationObserver) {
        var counter = 1, observer = new OriginalMutationObserver(handle), textNode = document.createTextNode(counter);
        observer.observe(textNode, {
            characterData: !0
        }), timerFunc = function() {
            counter = (counter + 1) % 2, textNode.data = counter;
        };
    } else timerFunc = window.setTimeout;
    context.setEndOfMicrotask = setEndOfMicrotask;
}(window.ShadowDOMPolyfill), function(scope) {
    "use strict";
    function scheduleCallback(observer) {
        observer.scheduled_ || (observer.scheduled_ = !0, globalMutationObservers.push(observer), 
        isScheduled || (setEndOfMicrotask(notifyObservers), isScheduled = !0));
    }
    function notifyObservers() {
        for (isScheduled = !1; globalMutationObservers.length; ) {
            var notifyList = globalMutationObservers;
            globalMutationObservers = [], notifyList.sort(function(x, y) {
            ...
            for (var i = 0; i < notifyList.length; i++) {
                ...
                removeTransientObserversFor(mo), queue.length && mo.callback_(queue, mo);
            }
        }
    }
    function MutationRecord(type, target) {
        this.type = type, this.target = target, this.addedNodes = new wrappers.NodeList(), 
        this.removedNodes = new wrappers.NodeList(), this.previousSibling = null, this.nextSibling = null, 
        this.attributeName = null, this.attributeNamespace = null, this.oldValue = null;
    }
    function registerTransientObservers(ancestor, node) {
        for (;ancestor; ancestor = ancestor.parentNode) {
            var registrations = registrationsTable.get(ancestor);
            if (registrations) for (var i = 0; i < registrations.length; i++) {
                var registration = registrations[i];
                registration.options.subtree && registration.addTransientObserver(node);
            }
        }
    }
    function removeTransientObserversFor(observer) {
        for (var i = 0; i < observer.nodes_.length; i++) {
            var node = observer.nodes_[i], registrations = registrationsTable.get(node);
    ...
    function enqueueMutation(target, type, data) {
        for (var interestedObservers = Object.create(null), associatedStrings = Object.create(null), node = target; node; node = node.parentNode) {
            ...
            if (registrations) for (var j = 0; j < registrations.length; j++) {
                ...
                if ((node === target || options.subtree) && !("attributes" === type && !options.attributes || "attributes" === type && options.attributeFilter && (null !== data.namespace || -1 === options.attributeFilter.indexOf(data.name)) || "characterData" === type && !options.characterData || "childList" === type && !options.childList)) {
                    var observer = registration.observer;
                    interestedObservers[observer.uid_] = observer, ("attributes" === type && options.attributeOldValue || "characterData" === type && options.characterDataOldValue) && (associatedStrings[observer.uid_] = data.oldValue);
                }
            }
        }
        for (var uid in interestedObservers) {
            var observer = interestedObservers[uid], record = new MutationRecord(type, target);
            "name" in data && "namespace" in data && (record.attributeName = data.name, record.attributeNamespace = data.namespace), 
            data.addedNodes && (record.addedNodes = data.addedNodes), data.removedNodes && (record.removedNodes = data.removedNodes), 
            data.previousSibling && (record.previousSibling = data.previousSibling), data.nextSibling && (record.nextSibling = data.nextSibling), 
            void 0 !== associatedStrings[uid] && (record.oldValue = associatedStrings[uid]), 
            scheduleCallback(observer), observer.records_.push(record);
        }
    }
    function MutationObserverOptions(options) {
        if (this.childList = !!options.childList, this.subtree = !!options.subtree, this.attributes = "attributes" in options || !("attributeOldValue" in options || "attributeFilter" in options) ? !!options.attributes : !0, 
        this.characterData = "characterDataOldValue" in options && !("characterData" in options) ? !0 : !!options.characterData, 
        !this.attributes && (options.attributeOldValue || "attributeFilter" in options) || !this.characterData && options.characterDataOldValue) throw new TypeError();
        if (this.characterData = !!options.characterData, this.attributeOldValue = !!options.attributeOldValue, 
        this.characterDataOldValue = !!options.characterDataOldValue, "attributeFilter" in options) {
            if (null == options.attributeFilter || "object" != typeof options.attributeFilter) throw new TypeError();
    ...
    var setEndOfMicrotask = scope.setEndOfMicrotask, wrapIfNeeded = scope.wrapIfNeeded, wrappers = scope.wrappers, registrationsTable = new WeakMap(), globalMutationObservers = [], isScheduled = !1, slice = Array.prototype.slice, uidCounter = 0;
    MutationObserver.prototype = {
        constructor: MutationObserver,
        observe: function(target, options) {
            target = wrapIfNeeded(target);
            var registration, newOptions = new MutationObserverOptions(options), registrations = registrationsTable.get(target);
            registrations || registrationsTable.set(target, registrations = []);
            for (var i = 0; i < registrations.length; i++) registrations[i].observer === this && (registration = registrations[i], 
            registration.removeTransientObservers(), registration.options = newOptions);
            registration || (registration = new Registration(this, target, newOptions), registrations.push(registration), 
        ...
        addTransientObserver: function(node) {
            if (node !== this.target) {
                scheduleCallback(this.observer), this.transientObservedNodes.push(node);
                var registrations = registrationsTable.get(node);
        ...
        removeTransientObservers: function() {
            ...
            for (var i = 0; i < transientObservedNodes.length; i++) for (var node = transientObservedNodes[i], registrations = registrationsTable.get(node), j = 0; j < registrations.length; j++) if (registrations[j] === this) {
                registrations.splice(j, 1);
                break;
            }
        }
    }, scope.enqueueMutation = enqueueMutation, scope.registerTransientObservers = registerTransientObservers, 
    ...
    function setTreeScope(node, treeScope) {
        if (node.treeScope_ !== treeScope) {
            ...
            for (var sr = node.shadowRoot; sr; sr = sr.olderShadowRoot) sr.treeScope_.parent = treeScope;
            for (var child = node.firstChild; child; child = child.nextSibling) setTreeScope(child, treeScope);
        }
    }
    function getTreeScope(node) {
        if (node instanceof scope.wrappers.Window, node.treeScope_) return node.treeScope_;
        var treeScope, parent = node.parentNode;
        return treeScope = parent ? getTreeScope(parent) : new TreeScope(node, null), node.treeScope_ = treeScope;
    }
    TreeScope.prototype = {
        get renderer() {
            return this.root instanceof scope.wrappers.ShadowRoot ? scope.getRendererForHost(this.root.host) : null;
        },
        contains: function(treeScope) {
            for (;treeScope; treeScope = treeScope.parent) if (treeScope === this) return !0;
            return !1;
        }
    }, scope.TreeScope = TreeScope, scope.getTreeScope = getTreeScope, scope.setTreeScope = setTreeScope;
}(window.ShadowDOMPolyfill), function(scope) {
    "use strict";
    ...
    function getEventPath(node, event) {
        ...
        for (path.push(current); current; ) {
            var destinationInsertionPoints = getDestinationInsertionPoints(current);
            if (destinationInsertionPoints && destinationInsertionPoints.length > 0) {
                for (var i = 0; i < destinationInsertionPoints.length; i++) {
                    var insertionPoint = destinationInsertionPoints[i];
                    if (isShadowInsertionPoint(insertionPoint)) {
                        var shadowRoot = rootOfNode(insertionPoint), olderShadowRoot = shadowRoot.olderShadowRoot;
                        olderShadowRoot && path.push(olderShadowRoot);
                    }
                    path.push(insertionPoint);
                }
                current = destinationInsertionPoints[destinationInsertionPoints.length - 1];
            } else if (isShadowRoot(current)) {
                if (inSameTree(node, current) && eventMustBeStopped(event)) break;
                current = current.host, path.push(current);
            } else current = current.parentNode, current && path.push(current);
        }
        return path;
    }
    ...
    function eventRetargetting(path, currentTarget) {
        if (0 === path.length) return currentTarget;
        currentTarget instanceof wrappers.Window && (currentTarget = currentTarget.document);
        for (var currentTargetTree = getTreeScope(currentTarget), originalTarget = path[0], originalTargetTree = getTreeScope(originalTarget), relativeTargetTree = lowestCommonInclusiveAncestor(currentTargetTree, originalTargetTree), i = 0; i < path.length; i++) {
            var node = path[i];
    ...
    function getTreeScopeAncestors(treeScope) {
        for (var ancestors = []; treeScope; treeScope = treeScope.parent) ancestors.push(treeScope);
        return ancestors;
    }
    function lowestCommonInclusiveAncestor(tsA, tsB) {
        for (var ancestorsA = getTreeScopeAncestors(tsA), ancestorsB = getTreeScopeAncestors(tsB), result = null; ancestorsA.length > 0 && ancestorsB.length > 0; ) {
            var a = ancestorsA.pop(), b = ancestorsB.pop();
            if (a !== b) break;
            result = a;
        }
        return result;
    ...
    function relatedTargetResolution(event, currentTarget, relatedTarget) {
        currentTarget instanceof wrappers.Window && (currentTarget = currentTarget.document);
        var lowestCommonAncestorTree, currentTargetTree = getTreeScope(currentTarget), relatedTargetTree = getTreeScope(relatedTarget), relatedTargetEventPath = getEventPath(relatedTarget, event), lowestCommonAncestorTree = lowestCommonInclusiveAncestor(currentTargetTree, relatedTargetTree);
        lowestCommonAncestorTree || (lowestCommonAncestorTree = relatedTargetTree.root);
        for (var commonAncestorTree = lowestCommonAncestorTree; commonAncestorTree; commonAncestorTree = commonAncestorTree.parent) for (var i = 0; i < relatedTargetEventPath.length; i++) {
    ...
    function dispatchOriginalEvent(originalEvent) {
        if (!handledEventsTable.get(originalEvent) && (handledEventsTable.set(originalEvent, !0), 
        dispatchEvent(wrap(originalEvent), wrap(originalEvent.target)), pendingError)) {
            var err = pendingError;
            throw pendingError = null, err;
        }
    }
    ...
    function dispatchEvent(event, originalWrapperTarget) {
        ...
        currentlyDispatchingEvents.set(event, !0), scope.renderAllPending();
        var eventPath, overrideTarget, win;
        if (isLoadLikeEvent(event) && !event.bubbles) {
            var doc = originalWrapperTarget;
            doc instanceof wrappers.Document && (win = doc.defaultView) && (overrideTarget = doc, 
            eventPath = []);
        }
        if (!eventPath) if (originalWrapperTarget instanceof wrappers.Window) win = originalWrapperTarget, 
        eventPath = []; else if (eventPath = getEventPath(originalWrapperTarget, event), 
        !isLoadLikeEvent(event)) {
            var doc = eventPath[eventPath.length - 1];
            doc instanceof wrappers.Document && (win = doc.defaultView);
        }
        return eventPathTable.set(event, eventPath), dispatchCapturing(event, eventPath, win, overrideTarget) && dispatchAtTarget(event, eventPath, win, overrideTarget) && dispatchBubbling(event, eventPath, win, overrideTarget), 
        eventPhaseTable.set(event, NONE), currentTargetTable["delete"](event, null), currentlyDispatchingEvents["delete"](event), 
        event.defaultPrevented;
    }
    function dispatchCapturing(event, eventPath, win, overrideTarget) {
        var phase = CAPTURING_PHASE;
        if (win && !invoke(win, event, phase, eventPath, overrideTarget)) return !1;
        for (var i = eventPath.length - 1; i > 0; i--) if (!invoke(eventPath[i], event, phase, eventPath, overrideTarget)) return !1;
        return !0;
    }
    function dispatchAtTarget(event, eventPath, win, overrideTarget) {
        var phase = AT_TARGET, currentTarget = eventPath[0] || win;
        return invoke(currentTarget, event, phase, eventPath, overrideTarget);
    }
    function dispatchBubbling(event, eventPath, win, overrideTarget) {
        for (var phase = BUBBLING_PHASE, i = 1; i < eventPath.length; i++) if (!invoke(eventPath[i], event, phase, eventPath, overrideTarget)) return;
        win && eventPath.length > 0 && invoke(win, event, phase, eventPath, overrideTarget);
    }
    function invoke(currentTarget, event, phase, eventPath, overrideTarget) {
        ...
        if (target === currentTarget) {
            if (phase === CAPTURING_PHASE) return !0;
            phase === BUBBLING_PHASE && (phase = AT_TARGET);
        } else if (phase === BUBBLING_PHASE && !event.bubbles) return !0;
        if ("relatedTarget" in event) {
            var originalEvent = unwrap(event), unwrappedRelatedTarget = originalEvent.relatedTarget;
            if (unwrappedRelatedTarget) {
                if (unwrappedRelatedTarget instanceof Object && unwrappedRelatedTarget.addEventListener) {
                    var relatedTarget = wrap(unwrappedRelatedTarget), adjusted = relatedTargetResolution(event, currentTarget, relatedTarget);
        ...
        targetTable.set(event, target), currentTargetTable.set(event, currentTarget), listeners.depth++;
        for (var i = 0, len = listeners.length; len > i; i++) {
            var listener = listeners[i];
            if (listener.removed) anyRemoved = !0; else if (!(listener.type !== type || !listener.capture && phase === CAPTURING_PHASE || listener.capture && phase === BUBBLING_PHASE)) try {
                ...
                pendingError || (pendingError = ex);
            }
        }
        if (listeners.depth--, anyRemoved && 0 === listeners.depth) {
    ...
    function Event(type, options) {
        if (!(type instanceof OriginalEvent)) return wrap(constructEvent(OriginalEvent, "Event", type, options));
        var impl = type;
        return OriginalBeforeUnloadEvent || "beforeunload" !== impl.type || this instanceof BeforeUnloadEvent ? void setWrapper(impl, this) : new BeforeUnloadEvent(impl);
    }
    ...
    function registerGenericEvent(name, SuperEvent, prototype) {
        var OriginalEvent = window[name], GenericEvent = function(type, options) {
            return type instanceof OriginalEvent ? void setWrapper(type, this) : wrap(constructEvent(OriginalEvent, name, type, options));
        };
        if (GenericEvent.prototype = Object.create(SuperEvent.prototype), prototype && mixin(GenericEvent.prototype, prototype), 
        OriginalEvent) try {
    ...
    function getTargetToListenAt(wrapper) {
        return wrapper instanceof wrappers.ShadowRoot && (wrapper = wrapper.host), unwrap(wrapper);
    }
    function hasListener(node, type) {
        var listeners = listenersTable.get(node);
        if (listeners) for (var i = 0; i < listeners.length; i++) if (!listeners[i].removed && listeners[i].type === type) return !0;
        return !1;
    }
    function hasListenerInAncestors(target, type) {
        for (var node = unwrap(target); node; node = node.parentNode) if (hasListener(wrap(node), type)) return !0;
        return !1;
    }
    ...
    function elementFromPoint(self, document, x, y) {
        ...
        var path = getEventPath(element, null), idx = path.lastIndexOf(self);
        return -1 == idx ? null : (path = path.slice(0, idx), eventRetargetting(path, self));
    }
    ...
    OriginalEvent.prototype.polymerBlackList_ = {
        ...
        get path() {
            ...
            return eventPath ? eventPath.slice() : [];
        },
    ...
    var UIEvent = registerGenericEvent("UIEvent", Event), CustomEvent = registerGenericEvent("CustomEvent", Event), relatedTargetProto = {
        get relatedTarget() {
            ...
            return void 0 !== relatedTarget ? relatedTarget : wrap(unwrap(this).relatedTarget);
        }
    }, mouseEventProto = mixin({
        initMouseEvent: getInitFunction("initMouseEvent", 14)
    }, relatedTargetProto), focusEventProto = mixin({
        initFocusEvent: getInitFunction("initFocusEvent", 5)
    }, relatedTargetProto), MouseEvent = registerGenericEvent("MouseEvent", UIEvent, mouseEventProto), FocusEvent = registerGenericEvent("FocusEvent", UIEvent, focusEventProto), defaultInitDicts = Object.create(null), supportsEventConstructors = function() {
        try {
    ...
    BeforeUnloadEvent.prototype = Object.create(Event.prototype), mixin(BeforeUnloadEvent.prototype, {
        get returnValue() {
    ...
    }), OriginalBeforeUnloadEvent && registerWrapper(OriginalBeforeUnloadEvent, BeforeUnloadEvent);
    ...
    [ Node, Window ].forEach(function(constructor) {
        ...
        addEventListener: function(type, fun, capture) {
            if (isValidListener(fun) && !isMutationEvent(type)) {
                ...
                } else listeners = [], listeners.depth = 0, listenersTable.set(this, listeners);
        ...
        dispatchEvent: function(event) {
            var nativeEvent = unwrap(event), eventType = nativeEvent.type;
            handledEventsTable.set(nativeEvent, !1), scope.renderAllPending();
            var tempListener;
            hasListenerInAncestors(this, eventType) || (tempListener = function() {}, this.addEventListener(eventType, tempListener, !0));
            try {
                return unwrap(this).dispatchEvent_(nativeEvent);
            } finally {
                tempListener && this.removeEventListener(eventType, tempListener, !0);
            }
        }
    }, OriginalEventTarget && registerWrapper(OriginalEventTarget, EventTarget);
    var originalElementFromPoint = document.elementFromPoint;
    scope.elementFromPoint = elementFromPoint, scope.getEventHandlerGetter = getEventHandlerGetter, 
    scope.getEventHandlerSetter = getEventHandlerSetter, scope.wrapEventTargetMethods = wrapEventTargetMethods, 
    scope.wrappers.BeforeUnloadEvent = BeforeUnloadEvent, scope.wrappers.CustomEvent = CustomEvent, 
    scope.wrappers.Event = Event, scope.wrappers.EventTarget = EventTarget, scope.wrappers.FocusEvent = FocusEvent, 
    scope.wrappers.MouseEvent = MouseEvent, scope.wrappers.UIEvent = UIEvent;
}(window.ShadowDOMPolyfill), function(scope) {
    "use strict";
    ...
    function wrapTouchList(nativeTouchList) {
        for (var list = new TouchList(), i = 0; i < nativeTouchList.length; i++) list[i] = new Touch(nativeTouchList[i]);
        return list.length = i, list;
    }
    function TouchEvent(impl) {
        UIEvent.call(this, impl);
    }
    var UIEvent = scope.wrappers.UIEvent, mixin = scope.mixin, registerWrapper = scope.registerWrapper, setWrapper = scope.setWrapper, unsafeUnwrap = scope.unsafeUnwrap, wrap = scope.wrap, OriginalTouchEvent = window.TouchEvent;
    if (OriginalTouchEvent) {
        ...
        }, TouchEvent.prototype = Object.create(UIEvent.prototype), mixin(TouchEvent.prototype, {
            get touches() {
        ...
        }), registerWrapper(OriginalTouchEvent, TouchEvent, nativeEvent), scope.wrappers.Touch = Touch, 
        scope.wrappers.TouchEvent = TouchEvent, scope.wrappers.TouchList = TouchList;
    }
}(window.ShadowDOMPolyfill), function(scope) {
    "use strict";
    function nonEnum(obj, prop) {
    ...
    var unsafeUnwrap = scope.unsafeUnwrap, wrap = scope.wrap, nonEnumDescriptor = {
        enumerable: !1
    };
    NodeList.prototype = {
        item: function(index) {
    ...
    "use strict";
    scope.wrapHTMLCollection = scope.wrapNodeList, scope.wrappers.HTMLCollection = scope.wrappers.NodeList;
}(window.ShadowDOMPolyfill), function(scope) {
    "use strict";
    ...
    function createOneElementNodeList(node) {
        ...
        return nodes[0] = node, nodes.length = 1, nodes;
    }
    ...
    function collectNodes(node, parentNode, previousNode, nextNode) {
        if (node instanceof DocumentFragment) {
            ...
            surpressMutations = !0;
            for (var i = nodes.length - 1; i >= 0; i--) node.removeChild(nodes[i]), nodes[i].parentNode_ = parentNode;
            surpressMutations = !1;
        ...
        var nodes = createOneElementNodeList(node), oldParent = node.parentNode;
        return oldParent && oldParent.removeChild(node), node.parentNode_ = parentNode, 
        node.previousSibling_ = previousNode, node.nextSibling_ = nextNode, previousNode && (previousNode.nextSibling_ = node), 
        nextNode && (nextNode.previousSibling_ = node), nodes;
    }
    function collectNodesNative(node) {
        if (node instanceof DocumentFragment) return collectNodesForDocumentFragment(node);
        var nodes = createOneElementNodeList(node), oldParent = node.parentNode;
        return oldParent && enqueueRemovalForInsertedNodes(node, oldParent, nodes), nodes;
    }
    function collectNodesForDocumentFragment(node) {
        for (var nodes = new NodeList(), i = 0, child = node.firstChild; child; child = child.nextSibling) nodes[i++] = child;
        return nodes.length = i, enqueueRemovalForInsertedDocumentFragment(node, nodes), 
        nodes;
    }
    ...
    function nodeWasAdded(node, treeScope) {
        setTreeScope(node, treeScope), node.nodeIsInserted_();
    }
    function nodesWereAdded(nodes, parent) {
        for (var treeScope = getTreeScope(parent), i = 0; i < nodes.length; i++) nodeWasAdded(nodes[i], treeScope);
    }
    function nodeWasRemoved(node) {
        setTreeScope(node, new TreeScope(node, null));
    }
    function nodesWereRemoved(nodes) {
        for (var i = 0; i < nodes.length; i++) nodeWasRemoved(nodes[i]);
    }
    function ensureSameOwnerDocument(parent, child) {
        var ownerDoc = parent.nodeType === Node.DOCUMENT_NODE ? parent : parent.ownerDocument;
        ownerDoc !== child.ownerDocument && ownerDoc.adoptNode(child);
    }
    function adoptNodesIfNeeded(owner, nodes) {
        if (nodes.length) {
            var ownerDoc = owner.ownerDocument;
            if (ownerDoc !== nodes[0].ownerDocument) for (var i = 0; i < nodes.length; i++) scope.adoptNodeNoRemove(nodes[i], ownerDoc);
        }
    }
    ...
    function clearChildNodes(wrapper) {
        if (void 0 !== wrapper.firstChild_) for (var child = wrapper.firstChild_; child; ) {
            var tmp = child;
            child = child.nextSibling_, tmp.parentNode_ = tmp.previousSibling_ = tmp.nextSibling_ = void 0;
        }
        wrapper.firstChild_ = wrapper.lastChild_ = void 0;
    }
    function removeAllChildNodes(wrapper) {
        ...
        } else for (var nextSibling, node = unwrap(wrapper), child = node.firstChild; child; ) nextSibling = child.nextSibling, 
        originalRemoveChild.call(node, child), child = nextSibling;
    }
    ...
    function cleanupNodes(nodes) {
        for (var n, i = 0; i < nodes.length; i++) n = nodes[i], n.parentNode.removeChild(n);
    }
    function cloneNode(node, deep, opt_doc) {
        var clone;
        if (clone = wrap(opt_doc ? originalImportNode.call(opt_doc, unsafeUnwrap(node), !1) : originalCloneNode.call(unsafeUnwrap(node), !1)), 
        deep) {
            for (var child = node.firstChild; child; child = child.nextSibling) clone.appendChild(cloneNode(child, !0, opt_doc));
            if (node instanceof wrappers.HTMLTemplateElement) for (var cloneContent = clone.content, child = node.content.firstChild; child; child = child.nextSibling) cloneContent.appendChild(cloneNode(child, !0, opt_doc));
        }
        return clone;
    }
    function contains(self, child) {
        if (!child || getTreeScope(self) !== getTreeScope(child)) return !1;
        for (var node = child; node; node = node.parentNode) if (node === self) return !0;
        return !1;
    }
    function Node(original) {
        ...
        this.previousSibling_ = void 0, this.treeScope_ = void 0;
    }
    ...
    OriginalNode.prototype.compareDocumentPosition), originalInsertBefore = OriginalNode.prototype.insertBefore, originalRemoveChild = OriginalNode.prototype.removeChild, originalReplaceChild = OriginalNode.prototype.replaceChild, isIe = /Trident|Edge/.test(navigator.userAgent), removeChildOriginalHelper = isIe ? function(parent, child) {
        try {
    ...
    Node.prototype = Object.create(EventTarget.prototype), mixin(Node.prototype, {
        ...
        insertBefore: function(childWrapper, refWrapper) {
            ...
            var refNode;
            refWrapper ? isWrapper(refWrapper) ? refNode = unwrap(refWrapper) : (refNode = refWrapper, 
            refWrapper = wrap(refNode)) : (refWrapper = null, refNode = null), refWrapper && assert(refWrapper.parentNode === this);
            var nodes, previousNode = refWrapper ? refWrapper.previousSibling : this.lastChild, useNative = !this.invalidateShadowRenderer() && !invalidateParent(childWrapper);
            if (nodes = useNative ? collectNodesNative(childWrapper) : collectNodes(childWrapper, this, previousNode, refWrapper), 
            useNative) ensureSameOwnerDocument(this, childWrapper), clearChildNodes(this), originalInsertBefore.call(unsafeUnwrap(this), unwrap(childWrapper), refNode); else {
                previousNode || (this.firstChild_ = nodes[0]), refWrapper || (this.lastChild_ = nodes[nodes.length - 1], 
                void 0 === this.firstChild_ && (this.firstChild_ = this.firstChild));
                var parentNode = refNode ? refNode.parentNode : unsafeUnwrap(this);
                parentNode ? originalInsertBefore.call(parentNode, unwrapNodesForInsertion(this, nodes), refNode) : adoptNodesIfNeeded(this, nodes);
            }
            return enqueueMutation(this, "childList", {
                addedNodes: nodes,
                nextSibling: refWrapper,
                previousSibling: previousNode
            }), nodesWereAdded(nodes, this), childWrapper;
        },
        removeChild: function(childWrapper) {
            if (assertIsNodeWrapper(childWrapper), childWrapper.parentNode !== this) {
                for (var found = !1, ieChild = (this.childNodes, this.firstChild); ieChild; ieChild = ieChild.nextSibling) if (ieChild === childWrapper) {
                    found = !0;
                    break;
                }
                if (!found) throw new Error("NotFoundError");
            }
            var childNode = unwrap(childWrapper), childWrapperNextSibling = childWrapper.nextSibling, childWrapperPreviousSibling = childWrapper.previousSibling;
            if (this.invalidateShadowRenderer()) {
                var thisFirstChild = this.firstChild, thisLastChild = this.lastChild, parentNode = childNode.parentNode;
                parentNode && removeChildOriginalHelper(parentNode, childNode), thisFirstChild === childWrapper && (this.firstChild_ = childWrapperNextSibling), 
                ...
                childWrapper.previousSibling_ = childWrapper.nextSibling_ = childWrapper.parentNode_ = void 0;
            } else clearChildNodes(this), removeChildOriginalHelper(unsafeUnwrap(this), childNode);
            return surpressMutations || enqueueMutation(this, "childList", {
                removedNodes: createOneElementNodeList(childWrapper),
                nextSibling: childWrapperNextSibling,
                previousSibling: childWrapperPreviousSibling
            }), registerTransientObservers(this, childWrapper), childWrapper;
        },
        replaceChild: function(newChildWrapper, oldChildWrapper) {
            assertIsNodeWrapper(newChildWrapper);
            var oldChildNode;
            if (isWrapper(oldChildWrapper) ? oldChildNode = unwrap(oldChildWrapper) : (oldChildNode = oldChildWrapper, 
            oldChildWrapper = wrap(oldChildNode)), oldChildWrapper.parentNode !== this) throw new Error("NotFoundError");
            var nodes, nextNode = oldChildWrapper.nextSibling, previousNode = oldChildWrapper.previousSibling, useNative = !this.invalidateShadowRenderer() && !invalidateParent(newChildWrapper);
            return useNative ? nodes = collectNodesNative(newChildWrapper) : (nextNode === newChildWrapper && (nextNode = newChildWrapper.nextSibling), 
            nodes = collectNodes(newChildWrapper, this, previousNode, nextNode)), useNative ? (ensureSameOwnerDocument(this, newChildWrapper), 
            clearChildNodes(this), originalReplaceChild.call(unsafeUnwrap(this), unwrap(newChildWrapper), oldChildNode)) : (this.firstChild === oldChildWrapper && (this.firstChild_ = nodes[0]), 
            this.lastChild === oldChildWrapper && (this.lastChild_ = nodes[nodes.length - 1]), 
            oldChildWrapper.previousSibling_ = oldChildWrapper.nextSibling_ = oldChildWrapper.parentNode_ = void 0, 
            oldChildNode.parentNode && originalReplaceChild.call(oldChildNode.parentNode, unwrapNodesForInsertion(this, nodes), oldChildNode)), 
            enqueueMutation(this, "childList", {
                addedNodes: nodes,
                removedNodes: createOneElementNodeList(oldChildWrapper),
                nextSibling: nextNode,
                previousSibling: previousNode
            }), nodeWasRemoved(oldChildWrapper), nodesWereAdded(nodes, this), oldChildWrapper;
        },
        nodeIsInserted_: function() {
            for (var child = this.firstChild; child; child = child.nextSibling) child.nodeIsInserted_();
        },
        hasChildNodes: function() {
            return null !== this.firstChild;
        },
        get parentNode() {
            return void 0 !== this.parentNode_ ? this.parentNode_ : wrap(unsafeUnwrap(this).parentNode);
        },
        get firstChild() {
            return void 0 !== this.firstChild_ ? this.firstChild_ : wrap(unsafeUnwrap(this).firstChild);
        },
        get lastChild() {
            return void 0 !== this.lastChild_ ? this.lastChild_ : wrap(unsafeUnwrap(this).lastChild);
        },
        get nextSibling() {
            return void 0 !== this.nextSibling_ ? this.nextSibling_ : wrap(unsafeUnwrap(this).nextSibling);
        },
        get previousSibling() {
            return void 0 !== this.previousSibling_ ? this.previousSibling_ : wrap(unsafeUnwrap(this).previousSibling);
        },
        get parentElement() {
        ...
        get textContent() {
            for (var s = "", child = this.firstChild; child; child = child.nextSibling) child.nodeType != Node.COMMENT_NODE && (s += child.textContent);
            return s;
        },
        set textContent(textContent) {
            ...
            if (this.invalidateShadowRenderer()) {
                if (removeAllChildNodes(this), "" !== textContent) {
                    var textNode = unsafeUnwrap(this).ownerDocument.createTextNode(textContent);
                    this.appendChild(textNode);
                }
            } else clearChildNodes(this), unsafeUnwrap(this).textContent = textContent;
            var addedNodes = snapshotNodeList(this.childNodes);
            enqueueMutation(this, "childList", {
                addedNodes: addedNodes,
                removedNodes: removedNodes
            }), nodesWereRemoved(removedNodes), nodesWereAdded(addedNodes, this);
        },
        get childNodes() {
        ...
        normalize: function() {
            for (var modNode, n, nodes = snapshotNodeList(this.childNodes), remNodes = [], s = "", i = 0; i < nodes.length; i++) n = nodes[i], 
            n.nodeType === Node.TEXT_NODE ? modNode || n.data.length ? modNode ? (s += n.data, 
            remNodes.push(n)) : modNode = n : this.removeChild(n) : (modNode && remNodes.length && (modNode.data += s, 
            cleanupNodes(remNodes)), remNodes = [], s = "", modNode = null, n.childNodes.length && n.normalize());
            modNode && remNodes.length && (modNode.data += s, cleanupNodes(remNodes));
        }
    ...
    scope.cloneNode = cloneNode, scope.nodeWasAdded = nodeWasAdded, scope.nodeWasRemoved = nodeWasRemoved, 
    scope.nodesWereAdded = nodesWereAdded, scope.nodesWereRemoved = nodesWereRemoved, 
    scope.originalInsertBefore = originalInsertBefore, scope.originalRemoveChild = originalRemoveChild, 
    scope.snapshotNodeList = snapshotNodeList, scope.wrappers.Node = Node;
}(window.ShadowDOMPolyfill), function(scope) {
    "use strict";
    function filterNodeList(list, index, result, deep) {
        for (var wrappedItem = null, root = null, i = 0, length = list.length; length > i; i++) wrappedItem = wrap(list[i]), 
        !deep && (root = getTreeScope(wrappedItem).root) && root instanceof scope.wrappers.ShadowRoot || (result[index++] = wrappedItem);
        return index;
    }
    ...
    function findElements(node, index, result, p, arg0, arg1) {
        for (var el = node.firstElementChild; el; ) p(el, arg0, arg1) && (result[index++] = el), 
        index = findElements(el, index, result, p, arg0, arg1), el = el.nextElementSibling;
        return index;
    }
    function querySelectorAllFiltered(p, index, result, selector, deep) {
        var list, target = unsafeUnwrap(this), root = getTreeScope(this).root;
        if (root instanceof scope.wrappers.ShadowRoot) return findElements(this, index, result, p, selector, null);
        if (target instanceof OriginalElement) list = originalElementQuerySelectorAll.call(target, selector); else {
            if (!(target instanceof OriginalDocument)) return findElements(this, index, result, p, selector, null);
    ...
    function getElementsByTagNameFiltered(p, index, result, localName, lowercase) {
        var list, target = unsafeUnwrap(this), root = getTreeScope(this).root;
        if (root instanceof scope.wrappers.ShadowRoot) return findElements(this, index, result, p, localName, lowercase);
        if (target instanceof OriginalElement) list = originalElementGetElementsByTagName.call(target, localName, lowercase); else {
            if (!(target instanceof OriginalDocument)) return findElements(this, index, result, p, localName, lowercase);
            list = originalDocumentGetElementsByTagName.call(target, localName, lowercase);
        }
        return filterNodeList(list, index, result, !1);
    }
    function getElementsByTagNameNSFiltered(p, index, result, ns, localName) {
        var list, target = unsafeUnwrap(this), root = getTreeScope(this).root;
        if (root instanceof scope.wrappers.ShadowRoot) return findElements(this, index, result, p, ns, localName);
        if (target instanceof OriginalElement) list = originalElementGetElementsByTagNameNS.call(target, ns, localName); else {
            if (!(target instanceof OriginalDocument)) return findElements(this, index, result, p, ns, localName);
            list = originalDocumentGetElementsByTagNameNS.call(target, ns, localName);
        }
        return filterNodeList(list, index, result, !1);
    }
    var HTMLCollection = scope.wrappers.HTMLCollection, NodeList = scope.wrappers.NodeList, getTreeScope = scope.getTreeScope, unsafeUnwrap = scope.unsafeUnwrap, wrap = scope.wrap, originalDocumentQuerySelector = document.querySelector, originalElementQuerySelector = document.documentElement.querySelector, originalDocumentQuerySelectorAll = document.querySelectorAll, originalElementQuerySelectorAll = document.documentElement.querySelectorAll, originalDocumentGetElementsByTagName = document.getElementsByTagName, originalElementGetElementsByTagName = document.documentElement.getElementsByTagName, originalDocumentGetElementsByTagNameNS = document.getElementsByTagNameNS, originalElementGetElementsByTagNameNS = document.documentElement.getElementsByTagNameNS, OriginalElement = window.Element, OriginalDocument = window.HTMLDocument || window.Document, XHTML_NS = "http://www.w3.org/1999/xhtml", SelectorsInterface = {
        querySelector: function(selector) {
            var shimmed = shimSelector(selector), deep = shimmed !== selector;
            selector = shimmed;
            var wrappedItem, target = unsafeUnwrap(this), root = getTreeScope(this).root;
            if (root instanceof scope.wrappers.ShadowRoot) return findOne(this, selector);
            if (target instanceof OriginalElement) wrappedItem = wrap(originalElementQuerySelector.call(target, selector)); else {
                if (!(target instanceof OriginalDocument)) return findOne(this, selector);
                wrappedItem = wrap(originalDocumentQuerySelector.call(target, selector));
            }
            return wrappedItem && !deep && (root = getTreeScope(wrappedItem).root) && root instanceof scope.wrappers.ShadowRoot ? findOne(this, selector) : wrappedItem;
        },
        querySelectorAll: function(selector) {
            var shimmed = shimSelector(selector), deep = shimmed !== selector;
            selector = shimmed;
            var result = new NodeList();
            return result.length = querySelectorAllFiltered.call(this, matchesSelector, 0, result, selector, deep), 
            result;
        }
    }, MatchesInterface = {
        matches: function(selector) {
            return selector = shimMatchesSelector(selector), scope.originalMatches.call(unsafeUnwrap(this), selector);
        }
    }, GetElementsByInterface = {
        getElementsByTagName: function(localName) {
            var result = new HTMLCollection(), match = "*" === localName ? matchesEveryThing : matchesTagName;
            return result.length = getElementsByTagNameFiltered.call(this, match, 0, result, localName, localName.toLowerCase()), 
            result;
        },
        ...
        getElementsByTagNameNS: function(ns, localName) {
            var result = new HTMLCollection(), match = null;
            return match = "*" === ns ? "*" === localName ? matchesEveryThing : matchesLocalNameOnly : "*" === localName ? matchesNameSpace : matchesLocalNameNS, 
    ...
    var ChildNodeInterface = scope.ChildNodeInterface, Node = scope.wrappers.Node, enqueueMutation = scope.enqueueMutation, mixin = scope.mixin, registerWrapper = scope.registerWrapper, unsafeUnwrap = scope.unsafeUnwrap, OriginalCharacterData = window.CharacterData;
    ...
    var CharacterData = scope.wrappers.CharacterData, mixin = (scope.enqueueMutation, 
    scope.mixin), registerWrapper = scope.registerWrapper, OriginalText = window.Text;
    Text.prototype = Object.create(CharacterData.prototype), mixin(Text.prototype, {
        splitText: function(offset) {
            offset = toUInt32(offset);
            var s = this.data;
            if (offset > s.length) throw new Error("IndexSizeError");
            var head = s.slice(0, offset), tail = s.slice(offset);
            this.data = head;
            var newTextNode = this.ownerDocument.createTextNode(tail);
            return this.parentNode && this.parentNode.insertBefore(newTextNode, this.nextSibling), 
            newTextNode;
        }
    }), registerWrapper(OriginalText, Text, document.createTextNode("")), scope.wrappers.Text = Text;
}(window.ShadowDOMPolyfill), function(scope) {
    "use strict";
    ...
    function changeClass(tokenList, method, args) {
        ...
        if (null == ownerElement) return method.apply(tokenList, args);
        var oldValue = getClass(ownerElement), retv = method.apply(tokenList, args);
        return getClass(ownerElement) !== oldValue && (enqueueClassAttributeChange(ownerElement, oldValue), 
        invalidateClass(ownerElement)), retv;
    }
    ...
    var unsafeUnwrap = scope.unsafeUnwrap, enqueueMutation = scope.enqueueMutation, oldAdd = DOMTokenList.prototype.add;
    ...
    function invalidateRendererBasedOnAttribute(element, name) {
        ...
        if (p && p.shadowRoot) {
            ...
            renderer.dependsOnAttribute(name) && renderer.invalidate();
        }
    }
    ...
    var ChildNodeInterface = scope.ChildNodeInterface, GetElementsByInterface = scope.GetElementsByInterface, Node = scope.wrappers.Node, ParentNodeInterface = scope.ParentNodeInterface, SelectorsInterface = scope.SelectorsInterface, MatchesInterface = scope.MatchesInterface, enqueueMutation = (scope.addWrapNodeListMethod, 
    scope.enqueueMutation), mixin = scope.mixin, registerWrapper = (scope.oneOf, scope.registerWrapper), unsafeUnwrap = scope.unsafeUnwrap, wrappers = scope.wrappers, OriginalElement = window.Element, matchesNames = [ "matches", "mozMatchesSelector", "msMatchesSelector", "webkitMatchesSelector" ].filter(function(name) {
        return OriginalElement.prototype[name];
    }), matchesName = matchesNames[0], originalMatches = OriginalElement.prototype[matchesName], classListTable = new WeakMap();
    Element.prototype = Object.create(Node.prototype), mixin(Element.prototype, {
        createShadowRoot: function() {
            var newShadowRoot = new wrappers.ShadowRoot(this);
            unsafeUnwrap(this).polymerShadowRoot_ = newShadowRoot;
            var renderer = scope.getRendererForHost(this);
            return renderer.invalidate(), newShadowRoot;
        },
        get shadowRoot() {
        ...
        setAttribute: function(name, value) {
            ...
            unsafeUnwrap(this).setAttribute(name, value), enqueAttributeChange(this, name, oldValue), 
            invalidateRendererBasedOnAttribute(this, name);
        },
        removeAttribute: function(name) {
            var oldValue = unsafeUnwrap(this).getAttribute(name);
            unsafeUnwrap(this).removeAttribute(name), enqueAttributeChange(this, name, oldValue), 
            invalidateRendererBasedOnAttribute(this, name);
        },
        get classList() {
            var list = classListTable.get(this);
            if (!list) {
                if (list = unsafeUnwrap(this).classList, !list) return;
                list.ownerElement_ = this, classListTable.set(this, list);
            }
            return list;
        },
        get className() {
        ...
        "matches" !== name && (Element.prototype[name] = function(selector) {
    ...
    mixin(Element.prototype, MatchesInterface), registerWrapper(OriginalElement, Element, document.createElementNS(null, "x")), 
    scope.invalidateRendererBasedOnAttribute = invalidateRendererBasedOnAttribute, scope.matchesNames = matchesNames, 
    scope.originalMatches = originalMatches, scope.wrappers.Element = Element;
}(window.ShadowDOMPolyfill), function(scope) {
    "use strict";
    ...
    function getOuterHTML(node, parentNode) {
        switch (node.nodeType) {
          case Node.ELEMENT_NODE:
            for (var attr, tagName = node.tagName.toLowerCase(), s = "<" + tagName, attrs = node.attributes, i = 0; attr = attrs[i]; i++) s += " " + attr.name + '="' + escapeAttr(attr.value) + '"';
    ...
    function getInnerHTML(node) {
        node instanceof wrappers.HTMLTemplateElement && (node = node.content);
        for (var s = "", child = node.firstChild; child; child = child.nextSibling) s += getOuterHTML(child, node);
        return s;
    }
    ...
    function frag(contextElement, html) {
        var p = unwrap(contextElement.cloneNode(!1));
        p.innerHTML = html;
        for (var c, df = unwrap(document.createDocumentFragment()); c = p.firstChild; ) df.appendChild(c);
        return wrap(df);
    }
    function getter(name) {
        return function() {
            return scope.renderAllPending(), unsafeUnwrap(this)[name];
        };
    }
    ...
    function getterAndSetterRequiresRendering(name) {
        Object.defineProperty(HTMLElement.prototype, name, {
            get: getter(name),
            set: function(v) {
                scope.renderAllPending(), unsafeUnwrap(this)[name] = v;
            },
            configurable: !0,
            enumerable: !0
        });
    ...
    function methodRequiresRendering(name) {
        Object.defineProperty(HTMLElement.prototype, name, {
            value: function() {
                return scope.renderAllPending(), unsafeUnwrap(this)[name].apply(unsafeUnwrap(this), arguments);
            },
            configurable: !0,
            enumerable: !0
        });
    }
    var Element = scope.wrappers.Element, defineGetter = scope.defineGetter, enqueueMutation = scope.enqueueMutation, mixin = scope.mixin, nodesWereAdded = scope.nodesWereAdded, nodesWereRemoved = scope.nodesWereRemoved, registerWrapper = scope.registerWrapper, snapshotNodeList = scope.snapshotNodeList, unsafeUnwrap = scope.unsafeUnwrap, unwrap = scope.unwrap, wrap = scope.wrap, wrappers = scope.wrappers, escapeAttrRegExp = /[&\u00A0"]/g, escapeDataRegExp = /[&\u00A0<>]/g, voidElements = makeSet([ "area", "base", "br", "col", "command", "embed", "hr", "img", "input", "keygen", "link", "meta", "param", "source", "track", "wbr" ]), plaintextParents = makeSet([ "style", "script", "xmp", "iframe", "noembed", "noframes", "plaintext", "noscript" ]), oldIe = /MSIE/.test(navigator.userAgent), OriginalHTMLElement = window.HTMLElement, OriginalHTMLTemplateElement = window.HTMLTemplateElement;
    HTMLElement.prototype = Object.create(Element.prototype), mixin(HTMLElement.prototype, {
        get innerHTML() {
            return getInnerHTML(this);
        },
        set innerHTML(value) {
            if (oldIe && plaintextParents[this.localName]) return void (this.textContent = value);
            var removedNodes = snapshotNodeList(this.childNodes);
            this.invalidateShadowRenderer() ? this instanceof wrappers.HTMLTemplateElement ? setInnerHTML(this.content, value) : setInnerHTML(this, value, this.tagName) : !OriginalHTMLTemplateElement && this instanceof wrappers.HTMLTemplateElement ? setInnerHTML(this.content, value) : unsafeUnwrap(this).innerHTML = value;
            var addedNodes = snapshotNodeList(this.childNodes);
            enqueueMutation(this, "childList", {
                addedNodes: addedNodes,
                removedNodes: removedNodes
        ...
        insertAdjacentHTML: function(position, text) {
            ...
            switch (String(position).toLowerCase()) {
              case "beforebegin":
                contextElement = this.parentNode, refNode = this;
                break;

              case "afterend":
                contextElement = this.parentNode, refNode = this.nextSibling;
                break;

              case "afterbegin":
                contextElement = this, refNode = this.firstChild;
                break;

              case "beforeend":
                contextElement = this, refNode = null;
                break;
    ...
    [ "scrollLeft", "scrollTop" ].forEach(getterAndSetterRequiresRendering), [ "getBoundingClientRect", "getClientRects", "scrollIntoView" ].forEach(methodRequiresRendering), 
    ...
    var HTMLElement = scope.wrappers.HTMLElement, mixin = scope.mixin, registerWrapper = scope.registerWrapper, unsafeUnwrap = scope.unsafeUnwrap, wrap = scope.wrap, OriginalHTMLCanvasElement = window.HTMLCanvasElement;
    HTMLCanvasElement.prototype = Object.create(HTMLElement.prototype), mixin(HTMLCanvasElement.prototype, {
        getContext: function() {
    ...
    var HTMLElement = scope.wrappers.HTMLElement, mixin = scope.mixin, registerWrapper = scope.registerWrapper, wrapHTMLCollection = scope.wrapHTMLCollection, unwrap = scope.unwrap, OriginalHTMLFormElement = window.HTMLFormElement;
    HTMLFormElement.prototype = Object.create(HTMLElement.prototype), mixin(HTMLFormElement.prototype, {
        get elements() {
    ...
    function Image(width, height) {
        if (!(this instanceof Image)) throw new TypeError("DOM object constructor cannot be called as a function.");
        var node = unwrap(document.createElement("img"));
        HTMLElement.call(this, node), rewrap(node, this), void 0 !== width && (node.width = width), 
        void 0 !== height && (node.height = height);
    }
    var HTMLElement = scope.wrappers.HTMLElement, registerWrapper = scope.registerWrapper, unwrap = scope.unwrap, rewrap = scope.rewrap, OriginalHTMLImageElement = window.HTMLImageElement;
    HTMLImageElement.prototype = Object.create(HTMLElement.prototype), registerWrapper(OriginalHTMLImageElement, HTMLImageElement, document.createElement("img")), 
    Image.prototype = HTMLImageElement.prototype, scope.wrappers.HTMLImageElement = HTMLImageElement, 
    ...
    var HTMLElement = scope.wrappers.HTMLElement, registerWrapper = (scope.mixin, scope.wrappers.NodeList, 
    scope.registerWrapper), OriginalHTMLShadowElement = window.HTMLShadowElement;
    HTMLShadowElement.prototype = Object.create(HTMLElement.prototype), HTMLShadowElement.prototype.constructor = HTMLShadowElement, 
    ...
    function extractContent(templateElement) {
        for (var child, doc = getTemplateContentsOwner(templateElement.ownerDocument), df = unwrap(doc.createDocumentFragment()); child = templateElement.firstChild; ) df.appendChild(child);
        return df;
    }
    function HTMLTemplateElement(node) {
        if (HTMLElement.call(this, node), !OriginalHTMLTemplateElement) {
            var content = extractContent(node);
            contentTable.set(this, wrap(content));
        }
    }
    var HTMLElement = scope.wrappers.HTMLElement, mixin = scope.mixin, registerWrapper = scope.registerWrapper, unsafeUnwrap = scope.unsafeUnwrap, unwrap = scope.unwrap, wrap = scope.wrap, contentTable = new WeakMap(), templateContentsOwnerTable = new WeakMap(), OriginalHTMLTemplateElement = window.HTMLTemplateElement;
    HTMLTemplateElement.prototype = Object.create(HTMLElement.prototype), mixin(HTMLTemplateElement.prototype, {
        constructor: HTMLTemplateElement,
        get content() {
            return OriginalHTMLTemplateElement ? wrap(unsafeUnwrap(this).content) : contentTable.get(this);
        }
    ...
    var HTMLElement = scope.wrappers.HTMLElement, registerWrapper = scope.registerWrapper, OriginalHTMLMediaElement = window.HTMLMediaElement;
    OriginalHTMLMediaElement && (HTMLMediaElement.prototype = Object.create(HTMLElement.prototype), 
    ...
    function Audio(src) {
        if (!(this instanceof Audio)) throw new TypeError("DOM object constructor cannot be called as a function.");
        var node = unwrap(document.createElement("audio"));
        HTMLMediaElement.call(this, node), rewrap(node, this), node.setAttribute("preload", "auto"), 
        void 0 !== src && node.setAttribute("src", src);
    }
    var HTMLMediaElement = scope.wrappers.HTMLMediaElement, registerWrapper = scope.registerWrapper, unwrap = scope.unwrap, rewrap = scope.rewrap, OriginalHTMLAudioElement = window.HTMLAudioElement;
    OriginalHTMLAudioElement && (HTMLAudioElement.prototype = Object.create(HTMLMediaElement.prototype), 
    registerWrapper(OriginalHTMLAudioElement, HTMLAudioElement, document.createElement("audio")), 
    Audio.prototype = HTMLAudioElement.prototype, scope.wrappers.HTMLAudioElement = HTMLAudioElement, 
    ...
    function Option(text, value, defaultSelected, selected) {
        if (!(this instanceof Option)) throw new TypeError("DOM object constructor cannot be called as a function.");
        var node = unwrap(document.createElement("option"));
        HTMLElement.call(this, node), rewrap(node, this), void 0 !== text && (node.text = text), 
        void 0 !== value && node.setAttribute("value", value), defaultSelected === !0 && node.setAttribute("selected", ""), 
        node.selected = selected === !0;
    }
    var HTMLElement = scope.wrappers.HTMLElement, mixin = scope.mixin, registerWrapper = scope.registerWrapper, rewrap = scope.rewrap, unwrap = scope.unwrap, wrap = scope.wrap, OriginalHTMLOptionElement = window.HTMLOptionElement;
    HTMLOptionElement.prototype = Object.create(HTMLElement.prototype), mixin(HTMLOptionElement.prototype, {
        get text() {
    ...
    Option.prototype = HTMLOptionElement.prototype, scope.wrappers.HTMLOptionElement = HTMLOptionElement, 
    ...
    var HTMLElement = scope.wrappers.HTMLElement, mixin = scope.mixin, registerWrapper = scope.registerWrapper, unwrap = scope.unwrap, wrap = scope.wrap, OriginalHTMLSelectElement = window.HTMLSelectElement;
    HTMLSelectElement.prototype = Object.create(HTMLElement.prototype), mixin(HTMLSelectElement.prototype, {
        add: function(element, before) {
            "object" == typeof before && (before = unwrap(before)), unwrap(this).add(unwrap(element), before);
        },
        remove: function(indexOrNode) {
            return void 0 === indexOrNode ? void HTMLElement.prototype.remove.call(this) : ("object" == typeof indexOrNode && (indexOrNode = unwrap(indexOrNode)), 
            void unwrap(this).remove(indexOrNode));
        },
        get form() {
            return wrap(unwrap(this).form);
        }
    ...
    var HTMLElement = scope.wrappers.HTMLElement, mixin = scope.mixin, registerWrapper = scope.registerWrapper, unwrap = scope.unwrap, wrap = scope.wrap, wrapHTMLCollection = scope.wrapHTMLCollection, OriginalHTMLTableElement = window.HTMLTableElement;
    HTMLTableElement.prototype = Object.create(HTMLElement.prototype), mixin(HTMLTableElement.prototype, {
        get caption() {
    ...
    var HTMLElement = scope.wrappers.HTMLElement, mixin = scope.mixin, registerWrapper = scope.registerWrapper, wrapHTMLCollection = scope.wrapHTMLCollection, unwrap = scope.unwrap, wrap = scope.wrap, OriginalHTMLTableSectionElement = window.HTMLTableSectionElement;
    HTMLTableSectionElement.prototype = Object.create(HTMLElement.prototype), mixin(HTMLTableSectionElement.prototype, {
    ...
    var HTMLElement = scope.wrappers.HTMLElement, mixin = scope.mixin, registerWrapper = scope.registerWrapper, wrapHTMLCollection = scope.wrapHTMLCollection, unwrap = scope.unwrap, wrap = scope.wrap, OriginalHTMLTableRowElement = window.HTMLTableRowElement;
    HTMLTableRowElement.prototype = Object.create(HTMLElement.prototype), mixin(HTMLTableRowElement.prototype, {
        get cells() {
    ...
    var HTMLContentElement = scope.wrappers.HTMLContentElement, HTMLElement = scope.wrappers.HTMLElement, HTMLShadowElement = scope.wrappers.HTMLShadowElement, HTMLTemplateElement = scope.wrappers.HTMLTemplateElement, registerWrapper = (scope.mixin, 
    scope.registerWrapper), OriginalHTMLUnknownElement = window.HTMLUnknownElement;
    ...
    "use strict";
    var Element = scope.wrappers.Element, HTMLElement = scope.wrappers.HTMLElement, registerObject = scope.registerObject, defineWrapGetter = scope.defineWrapGetter, SVG_NS = "http://www.w3.org/2000/svg", svgTitleElement = document.createElementNS(SVG_NS, "title"), SVGTitleElement = registerObject(svgTitleElement), SVGElement = Object.getPrototypeOf(SVGTitleElement.prototype).constructor;
    if (!("classList" in svgTitleElement)) {
        var descr = Object.getOwnPropertyDescriptor(Element.prototype, "classList");
        Object.defineProperty(HTMLElement.prototype, "classList", descr), delete Element.prototype.classList;
    }
    defineWrapGetter(SVGElement, "ownerSVGElement"), scope.wrappers.SVGElement = SVGElement;
}(window.ShadowDOMPolyfill), function(scope) {
    "use strict";
    function SVGUseElement(impl) {
        parentInterface.call(this, impl);
    }
    var mixin = scope.mixin, registerWrapper = scope.registerWrapper, unwrap = scope.unwrap, wrap = scope.wrap, OriginalSVGUseElement = window.SVGUseElement, SVG_NS = "http://www.w3.org/2000/svg", gWrapper = wrap(document.createElementNS(SVG_NS, "g")), useElement = document.createElementNS(SVG_NS, "use"), SVGGElement = gWrapper.constructor, parentInterfacePrototype = Object.getPrototypeOf(SVGGElement.prototype), parentInterface = parentInterfacePrototype.constructor;
    SVGUseElement.prototype = Object.create(parentInterfacePrototype), "instanceRoot" in useElement && mixin(SVGUseElement.prototype, {
        get instanceRoot() {
    ...
    }), registerWrapper(OriginalSVGUseElement, SVGUseElement, useElement), scope.wrappers.SVGUseElement = SVGUseElement;
}(window.ShadowDOMPolyfill), function(scope) {
    "use strict";
    function SVGElementInstance(impl) {
        EventTarget.call(this, impl);
    }
    var EventTarget = scope.wrappers.EventTarget, mixin = scope.mixin, registerWrapper = scope.registerWrapper, unsafeUnwrap = scope.unsafeUnwrap, wrap = scope.wrap, OriginalSVGElementInstance = window.SVGElementInstance;
    OriginalSVGElementInstance && (SVGElementInstance.prototype = Object.create(EventTarget.prototype), 
    ...
    }), registerWrapper(OriginalSVGElementInstance, SVGElementInstance), scope.wrappers.SVGElementInstance = SVGElementInstance);
}(window.ShadowDOMPolyfill), function(scope) {
    "use strict";
    function CanvasRenderingContext2D(impl) {
        setWrapper(impl, this);
    }
    var mixin = scope.mixin, registerWrapper = scope.registerWrapper, setWrapper = scope.setWrapper, unsafeUnwrap = scope.unsafeUnwrap, unwrap = scope.unwrap, unwrapIfNeeded = scope.unwrapIfNeeded, wrap = scope.wrap, OriginalCanvasRenderingContext2D = window.CanvasRenderingContext2D;
    mixin(CanvasRenderingContext2D.prototype, {
        get canvas() {
            return wrap(unsafeUnwrap(this).canvas);
        },
        drawImage: function() {
            arguments[0] = unwrapIfNeeded(arguments[0]), unsafeUnwrap(this).drawImage.apply(unsafeUnwrap(this), arguments);
        },
        createPattern: function() {
            return arguments[0] = unwrap(arguments[0]), unsafeUnwrap(this).createPattern.apply(unsafeUnwrap(this), arguments);
        }
    ...
    var mixin = scope.mixin, registerWrapper = scope.registerWrapper, setWrapper = scope.setWrapper, unsafeUnwrap = scope.unsafeUnwrap, unwrapIfNeeded = scope.unwrapIfNeeded, wrap = scope.wrap, OriginalWebGLRenderingContext = window.WebGLRenderingContext;
    if (OriginalWebGLRenderingContext) {
        mixin(WebGLRenderingContext.prototype, {
            ...
            texImage2D: function() {
                arguments[5] = unwrapIfNeeded(arguments[5]), unsafeUnwrap(this).texImage2D.apply(unsafeUnwrap(this), arguments);
            },
            texSubImage2D: function() {
                arguments[6] = unwrapIfNeeded(arguments[6]), unsafeUnwrap(this).texSubImage2D.apply(unsafeUnwrap(this), arguments);
            }
        });
    ...
    var registerWrapper = scope.registerWrapper, setWrapper = scope.setWrapper, unsafeUnwrap = scope.unsafeUnwrap, unwrap = scope.unwrap, unwrapIfNeeded = scope.unwrapIfNeeded, wrap = scope.wrap, OriginalRange = window.Range;
    Range.prototype = {
        get startContainer() {
    ...
    }, OriginalRange.prototype.createContextualFragment && (Range.prototype.createContextualFragment = function(html) {
        return wrap(unsafeUnwrap(this).createContextualFragment(html));
    }), registerWrapper(window.Range, Range, document.createRange()), scope.wrappers.Range = Range;
}(window.ShadowDOMPolyfill), function(scope) {
    "use strict";
    ...
    function ShadowRoot(hostWrapper) {
        ...
        nextOlderShadowTreeTable.set(this, oldShadowRoot), this.treeScope_ = new TreeScope(this, getTreeScope(oldShadowRoot || hostWrapper)), 
        shadowHostTable.set(this, hostWrapper);
    }
    var DocumentFragment = scope.wrappers.DocumentFragment, TreeScope = scope.TreeScope, elementFromPoint = scope.elementFromPoint, getInnerHTML = scope.getInnerHTML, getTreeScope = scope.getTreeScope, mixin = scope.mixin, rewrap = scope.rewrap, setInnerHTML = scope.setInnerHTML, unsafeUnwrap = scope.unsafeUnwrap, unwrap = scope.unwrap, shadowHostTable = new WeakMap(), nextOlderShadowTreeTable = new WeakMap(), spaceCharRe = /[ \t\n\r\f]/;
    ShadowRoot.prototype = Object.create(DocumentFragment.prototype), mixin(ShadowRoot.prototype, {
        ...
        getElementById: function(id) {
            return spaceCharRe.test(id) ? null : this.querySelector('[id="' + id + '"]');
        }
    ...
    function insertBefore(parentNodeWrapper, newChildWrapper, refChildWrapper) {
        var parentNode = unwrap(parentNodeWrapper), newChild = unwrap(newChildWrapper), refChild = refChildWrapper ? unwrap(refChildWrapper) : null;
        if (remove(newChildWrapper), updateWrapperUpAndSideways(newChildWrapper), refChildWrapper) parentNodeWrapper.firstChild === refChildWrapper && (parentNodeWrapper.firstChild_ = refChildWrapper), 
        refChildWrapper.previousSibling_ = refChildWrapper.previousSibling; else {
            parentNodeWrapper.lastChild_ = parentNodeWrapper.lastChild, parentNodeWrapper.lastChild === parentNodeWrapper.firstChild && (parentNodeWrapper.firstChild_ = parentNodeWrapper.firstChild);
    ...
    function remove(nodeWrapper) {
        ...
        if (parentNode) {
            ...
            updateWrapperUpAndSideways(nodeWrapper), nodeWrapper.previousSibling && (nodeWrapper.previousSibling.nextSibling_ = nodeWrapper), 
            nodeWrapper.nextSibling && (nodeWrapper.nextSibling.previousSibling_ = nodeWrapper), 
            parentNodeWrapper.lastChild === nodeWrapper && (parentNodeWrapper.lastChild_ = nodeWrapper), 
            parentNodeWrapper.firstChild === nodeWrapper && (parentNodeWrapper.firstChild_ = nodeWrapper), 
    ...
    function getDistributedNodes(insertionPoint) {
        ...
        return rv || distributedNodesTable.set(insertionPoint, rv = []), rv;
    }
    ...
    function renderAllPending() {
        for (var i = 0; i < pendingDirtyRenderers.length; i++) {
            var renderer = pendingDirtyRenderers[i], parentRenderer = renderer.parentRenderer;
            parentRenderer && parentRenderer.dirty || renderer.render();
        }
        pendingDirtyRenderers = [];
    }
    function handleRequestAnimationFrame() {
        renderTimer = null, renderAllPending();
    }
    ...
    function RenderNode(node) {
        this.skip = !1, this.node = node, this.childNodes = [];
    }
    function ShadowRenderer(host) {
        this.host = host, this.dirty = !1, this.invalidateAttributes(), this.associateNode(host);
    }
    function poolPopulation(node) {
        for (var pool = [], child = node.firstChild; child; child = child.nextSibling) isInsertionPoint(child) ? pool.push.apply(pool, getDistributedNodes(child)) : pool.push(child);
        return pool;
    }
    ...
    function destributeNodeInto(child, insertionPoint) {
        ...
        points ? points.push(insertionPoint) : destinationInsertionPointsTable.set(child, [ insertionPoint ]);
    }
    function getDestinationInsertionPoints(node) {
        return destinationInsertionPointsTable.get(node);
    }
    function resetDestinationInsertionPoints(node) {
        destinationInsertionPointsTable.set(node, void 0);
    }
    function matches(node, contentElement) {
        ...
        if (!selectorStartCharRe.test(select)) return !1;
        try {
    ...
    var renderTimer, Element = scope.wrappers.Element, HTMLContentElement = scope.wrappers.HTMLContentElement, HTMLShadowElement = scope.wrappers.HTMLShadowElement, Node = scope.wrappers.Node, ShadowRoot = scope.wrappers.ShadowRoot, getTreeScope = (scope.assert, 
    scope.getTreeScope), oneOf = (scope.mixin, scope.oneOf), unsafeUnwrap = scope.unsafeUnwrap, unwrap = scope.unwrap, wrap = scope.wrap, ArraySplice = scope.ArraySplice, distributedNodesTable = new WeakMap(), destinationInsertionPointsTable = new WeakMap(), rendererForHostTable = new WeakMap(), request = oneOf(window, [ "requestAnimationFrame", "mozRequestAnimationFrame", "webkitRequestAnimationFrame", "setTimeout" ]), pendingDirtyRenderers = [], spliceDiff = new ArraySplice();
    spliceDiff.equals = function(renderNode, rawNode) {
        return unwrap(renderNode.node) === rawNode;
    }, RenderNode.prototype = {
        append: function(node) {
            var rv = new RenderNode(node);
            return this.childNodes.push(rv), rv;
        },
        sync: function(opt_added) {
            if (!this.skip) {
                for (var nodeWrapper = this.node, newChildren = this.childNodes, oldChildren = getChildNodesSnapshot(unwrap(nodeWrapper)), added = opt_added || new WeakMap(), splices = spliceDiff.calculateSplices(newChildren, oldChildren), newIndex = 0, oldIndex = 0, lastIndex = 0, i = 0; i < splices.length; i++) {
                    for (var splice = splices[i]; lastIndex < splice.index; lastIndex++) oldIndex++, 
                    newChildren[newIndex++].sync(added);
                    for (var removedCount = splice.removed.length, j = 0; removedCount > j; j++) {
                        var wrapper = wrap(oldChildren[oldIndex++]);
                        added.get(wrapper) || remove(wrapper);
                    }
                    for (var addedCount = splice.addedCount, refNode = oldChildren[oldIndex] && wrap(oldChildren[oldIndex]), j = 0; addedCount > j; j++) {
                        var newChildRenderNode = newChildren[newIndex++], newChildWrapper = newChildRenderNode.node;
                        insertBefore(nodeWrapper, newChildWrapper, refNode), added.set(newChildWrapper, !0), 
                        newChildRenderNode.sync(added);
                    }
                    lastIndex += addedCount;
                }
                for (var i = lastIndex; i < newChildren.length; i++) newChildren[i].sync(added);
            }
        }
        ...
        render: function(opt_renderNode) {
            if (this.dirty) {
                ...
                topMostRenderer && renderNode.sync(), this.dirty = !1;
            }
        },
        get parentRenderer() {
        ...
        invalidate: function() {
            if (!this.dirty) {
                ...
                if (parentRenderer && parentRenderer.invalidate(), pendingDirtyRenderers.push(this), 
                renderTimer) return;
                renderTimer = window[request](handleRequestAnimationFrame, 0);
            }
        },
        distribution: function(root) {
            this.resetAllSubtrees(root), this.distributionResolution(root);
        },
        resetAll: function(node) {
            isInsertionPoint(node) ? resetDistributedNodes(node) : resetDestinationInsertionPoints(node), 
            this.resetAllSubtrees(node);
        },
        resetAllSubtrees: function(node) {
            for (var child = node.firstChild; child; child = child.nextSibling) this.resetAll(child);
            node.shadowRoot && this.resetAll(node.shadowRoot), node.olderShadowRoot && this.resetAll(node.olderShadowRoot);
        },
        distributionResolution: function(node) {
            if (isShadowHost(node)) {
                for (var shadowHost = node, pool = poolPopulation(shadowHost), shadowTrees = getShadowTrees(shadowHost), i = 0; i < shadowTrees.length; i++) this.poolDistribution(shadowTrees[i], pool);
                for (var i = shadowTrees.length - 1; i >= 0; i--) {
                    var shadowTree = shadowTrees[i], shadow = getShadowInsertionPoint(shadowTree);
                    if (shadow) {
                        var olderShadowRoot = shadowTree.olderShadowRoot;
                        olderShadowRoot && (pool = poolPopulation(olderShadowRoot));
                        for (var j = 0; j < pool.length; j++) destributeNodeInto(pool[j], shadow);
                    }
                    this.distributionResolution(shadowTree);
                }
            }
            for (var child = node.firstChild; child; child = child.nextSibling) this.distributionResolution(child);
        },
        poolDistribution: function(node, pool) {
            if (!(node instanceof HTMLShadowElement)) if (node instanceof HTMLContentElement) {
                var content = node;
                this.updateDependentAttributes(content.getAttribute("select"));
                for (var anyDistributed = !1, i = 0; i < pool.length; i++) {
                    var node = pool[i];
                    node && matches(node, content) && (destributeNodeInto(node, content), pool[i] = void 0, 
                    anyDistributed = !0);
                }
                if (!anyDistributed) for (var child = content.firstChild; child; child = child.nextSibling) destributeNodeInto(child, content);
            } else for (var child = node.firstChild; child; child = child.nextSibling) this.poolDistribution(child, pool);
        },
        buildRenderTree: function(renderNode, node) {
            for (var children = this.compose(node), i = 0; i < children.length; i++) {
                var child = children[i], childRenderNode = renderNode.append(child);
        ...
        compose: function(node) {
            for (var children = [], p = node.shadowRoot || node, child = p.firstChild; child; child = child.nextSibling) if (isInsertionPoint(child)) {
                this.associateNode(p);
                for (var distributedNodes = getDistributedNodes(child), j = 0; j < distributedNodes.length; j++) {
        ...
        updateDependentAttributes: function(selector) {
            if (selector) {
                ...
                /\.\w+/.test(selector) && (attributes["class"] = !0), /#\w+/.test(selector) && (attributes.id = !0), 
    ...
    Node.prototype.invalidateShadowRenderer = function() {
        ...
        return renderer ? (renderer.invalidate(), !0) : !1;
    }, HTMLContentElement.prototype.getDistributedNodes = HTMLShadowElement.prototype.getDistributedNodes = function() {
        return renderAllPending(), getDistributedNodes(this);
    }, Element.prototype.getDestinationInsertionPoints = function() {
        return renderAllPending(), getDestinationInsertionPoints(this) || [];
    }, HTMLContentElement.prototype.nodeIsInserted_ = HTMLShadowElement.prototype.nodeIsInserted_ = function() {
        this.invalidateShadowRenderer();
        var renderer, shadowRoot = getShadowRootAncestor(this);
        shadowRoot && (renderer = getRendererForShadowRoot(shadowRoot)), unsafeUnwrap(this).polymerShadowRenderer_ = renderer, 
        renderer && renderer.invalidate();
    }, scope.getRendererForHost = getRendererForHost, scope.getShadowTrees = getShadowTrees, 
    scope.renderAllPending = renderAllPending, scope.getDestinationInsertionPoints = getDestinationInsertionPoints, 
    scope.visual = {
    ...
    function createWrapperConstructor(name) {
        if (window[name]) {
            ...
            GeneratedWrapper.prototype = Object.create(HTMLElement.prototype), mixin(GeneratedWrapper.prototype, {
                get form() {
                    return wrap(unwrap(this).form);
                }
    ...
    {
        var registerWrapper = scope.registerWrapper, setWrapper = scope.setWrapper, unsafeUnwrap = scope.unsafeUnwrap, unwrap = scope.unwrap, unwrapIfNeeded = scope.unwrapIfNeeded, wrap = scope.wrap;
        window.Selection;
    }
    Selection.prototype = {
        get anchorNode() {
    ...
    }, registerWrapper(window.Selection, Selection, window.getSelection()), scope.wrappers.Selection = Selection;
}(window.ShadowDOMPolyfill), function(scope) {
    "use strict";
    function Document(node) {
        Node.call(this, node), this.treeScope_ = new TreeScope(this, null);
    }
    ...
    function adoptNodeNoRemove(node, doc) {
        originalAdoptNode.call(unsafeUnwrap(doc), unwrap(node)), adoptSubtree(node, doc);
    }
    ...
    var originalAdoptNode = document.adoptNode, originalGetSelection = document.getSelection;
    if (mixin(Document.prototype, {
        adoptNode: function(node) {
            return node.parentNode && node.parentNode.removeChild(node), adoptNodeNoRemove(node, this), 
            node;
        },
        elementFromPoint: function(x, y) {
        ...
        getSelection: function() {
            return renderAllPending(), new Selection(originalGetSelection.call(unwrap(this)));
        },
        ...
        Document.prototype.registerElement = function(tagName, object) {
            function CustomElementConstructor(node) {
                return node ? void setWrapper(node, this) : extendsOption ? document.createElement(extendsOption, tagName) : document.createElement(tagName);
            }
            var prototype, extendsOption;
            if (void 0 !== object && (prototype = object.prototype, extendsOption = object["extends"]), 
            prototype || (prototype = Object.create(HTMLElement.prototype)), scope.nativePrototypeTable.get(prototype)) throw new Error("NotSupportedError");
            ...
            [ "createdCallback", "attachedCallback", "detachedCallback", "attributeChangedCallback" ].forEach(function(name) {
                ...
                f && (newPrototype[name] = function() {
                    wrap(this) instanceof CustomElementConstructor || rewrap(this), f.apply(wrap(this), arguments);
                });
            });
            var p = {
                prototype: newPrototype
            };
            extendsOption && (p["extends"] = extendsOption), CustomElementConstructor.prototype = prototype, 
            CustomElementConstructor.prototype.constructor = CustomElementConstructor, scope.constructorTable.set(newPrototype, CustomElementConstructor), 
    ...
    scope.adoptNodeNoRemove = adoptNodeNoRemove, scope.wrappers.DOMImplementation = DOMImplementation, 
    ...
    Window.prototype = Object.create(EventTarget.prototype), OriginalWindow.prototype.getComputedStyle = function(el, pseudo) {
        return wrap(this || window).getComputedStyle(unwrapIfNeeded(el), pseudo);
    }, originalGetDefaultComputedStyle && (OriginalWindow.prototype.getDefaultComputedStyle = function(el, pseudo) {
    ...
    [ "addEventListener", "removeEventListener", "dispatchEvent" ].forEach(function(name) {
        ...
        getComputedStyle: function(el, pseudo) {
            return renderAllPending(), originalGetComputedStyle.call(unwrap(this), unwrapIfNeeded(el), pseudo);
        },
        getSelection: function() {
            return renderAllPending(), new Selection(originalGetSelection.call(unwrap(this)));
        },
    ...
    }), originalGetDefaultComputedStyle && (Window.prototype.getDefaultComputedStyle = function(el, pseudo) {
        return renderAllPending(), originalGetDefaultComputedStyle.call(unwrap(this), unwrapIfNeeded(el), pseudo);
    }), registerWrapper(OriginalWindow, Window, window), scope.wrappers.Window = Window;
}(window.ShadowDOMPolyfill), function(scope) {
    "use strict";
    var unwrap = scope.unwrap, OriginalDataTransfer = window.DataTransfer || window.Clipboard, OriginalDataTransferSetDragImage = OriginalDataTransfer.prototype.setDragImage;
    OriginalDataTransferSetDragImage && (OriginalDataTransfer.prototype.setDragImage = function(image, x, y) {
    ...
    function FormData(formElement) {
        var impl;
        impl = formElement instanceof OriginalFormData ? formElement : new OriginalFormData(formElement && unwrap(formElement)), 
        setWrapper(impl, this);
    }
    var registerWrapper = scope.registerWrapper, setWrapper = scope.setWrapper, unwrap = scope.unwrap, OriginalFormData = window.FormData;
    OriginalFormData && (registerWrapper(OriginalFormData, FormData, new OriginalFormData()), 
    scope.wrappers.FormData = FormData);
}(window.ShadowDOMPolyfill), function(scope) {
    "use strict";
    var unwrapIfNeeded = scope.unwrapIfNeeded, originalSend = XMLHttpRequest.prototype.send;
    ...
    var elements = (scope.isWrapperFor, {
        a: "HTMLAnchorElement",
    ...
    function cssTextToStyle(cssText) {
        ...
        return style.textContent = cssText, style;
    }
    function cssToRules(cssText) {
        ...
        } catch (e) {} else console.warn("sheet not found", style);
        return style.parentNode.removeChild(style), rules;
    }
    function initFrame() {
        frame.initialized = !0, document.body.appendChild(frame);
        var doc = frame.contentDocument, base = doc.createElement("base");
        base.href = document.baseURI, doc.head.appendChild(base);
    }
    function inFrame(fn) {
        frame.initialized || initFrame(), document.body.appendChild(frame), fn(frame.contentDocument), 
        ...
        if (callback) {
            ...
            if (cssText.match("@import") && isChrome) {
                ...
                inFrame(function(doc) {
                    doc.head.appendChild(style.impl), rules = Array.prototype.slice.call(style.sheet.cssRules, 0), 
                    callback(rules);
                });
            } else rules = cssToRules(cssText), callback(rules);
        }
    }
    ...
    function addOwnSheet(cssText, name) {
        var style = cssTextToStyle(cssText);
        style.setAttribute(name, ""), style.setAttribute(SHIMMED_ATTRIBUTE, ""), document.head.appendChild(style);
    }
    function getSheet() {
        return sheet || (sheet = document.createElement("style"), sheet.setAttribute(SHIMMED_ATTRIBUTE, ""), 
        sheet[SHIMMED_ATTRIBUTE] = !0), sheet;
    }
    var ShadowCSS = {
        strictStyling: !1,
        registry: {},
        shimStyling: function(root, name, extendsName) {
            var scopeStyles = this.prepareRoot(root, name, extendsName), typeExtension = this.isTypeExtension(extendsName), scopeSelector = this.makeScopeSelector(name, typeExtension), cssText = stylesToCssText(scopeStyles, !0);
            cssText = this.scopeCssText(cssText, scopeSelector), root && (root.shimmedStyle = cssText), 
        ...
        shimCssText: function(cssText, selector) {
            return cssText = this.insertDirectives(cssText), this.scopeCssText(cssText, selector);
        },
        makeScopeSelector: function(name, typeExtension) {
            return name ? typeExtension ? "[is=" + name + "]" : name : "";
        },
        ...
        prepareRoot: function(root, name, extendsName) {
            ...
            return this.replaceTextInStyles(def.rootStyles, this.insertDirectives), this.removeStyles(root, def.rootStyles), 
            this.strictStyling && this.applyScopeToContent(root, name), def.scopeStyles;
        },
        removeStyles: function(root, styles) {
            for (var s, i = 0, l = styles.length; l > i && (s = styles[i]); i++) s.parentNode.removeChild(s);
        },
        registerRoot: function(root, name, extendsName) {
            ...
            def.rootStyles = styles, def.scopeStyles = def.rootStyles;
            var extendee = this.registry[def.extendsName];
            return extendee && (def.scopeStyles = extendee.scopeStyles.concat(def.scopeStyles)), 
            def;
        },
        ...
        insertDirectives: function(cssText) {
            return cssText = this.insertPolyfillDirectivesInCssText(cssText), this.insertPolyfillRulesInCssText(cssText);
        },
        insertPolyfillDirectivesInCssText: function(cssText) {
            return cssText = cssText.replace(cssCommentNextSelectorRe, function(match, p1) {
                return p1.slice(0, -2) + "{";
            }), cssText.replace(cssContentNextSelectorRe, function(match, p1) {
                return p1 + " {";
            });
        },
        insertPolyfillRulesInCssText: function(cssText) {
            return cssText = cssText.replace(cssCommentRuleRe, function(match, p1) {
                return p1.slice(0, -1);
            }), cssText.replace(cssContentRuleRe, function(match, p1, p2, p3) {
        ...
        scopeCssText: function(cssText, scopeSelector) {
            ...
            if (cssText = this.insertPolyfillHostInCssText(cssText), cssText = this.convertColonHost(cssText), 
            cssText = this.convertColonHostContext(cssText), cssText = this.convertShadowDOMSelectors(cssText), 
            scopeSelector) {
        ...
        extractUnscopedRulesFromCssText: function(cssText) {
            for (var m, r = ""; m = cssCommentUnscopedRuleRe.exec(cssText); ) r += m[1].slice(0, -1) + "\n\n";
        ...
        convertColonRule: function(cssText, regExp, partReplacer) {
            return cssText.replace(regExp, function(m, p1, p2, p3) {
                if (p1 = polyfillHostNoCombinator, p2) {
                    for (var p, parts = p2.split(","), r = [], i = 0, l = parts.length; l > i && (p = parts[i]); i++) p = p.trim(), 
        ...
        colonHostContextPartReplacer: function(host, part, suffix) {
            return part.match(polyfillHost) ? this.colonHostPartReplacer(host, part, suffix) : host + part + suffix + ", " + part + " " + host + suffix;
        },
        colonHostPartReplacer: function(host, part, suffix) {
            return host + part.replace(polyfillHost, "") + suffix;
        },
        convertShadowDOMSelectors: function(cssText) {
            for (var i = 0; i < shadowDOMSelectorsRe.length; i++) cssText = cssText.replace(shadowDOMSelectorsRe[i], " ");
            return cssText;
        },
        scopeRules: function(cssRules, scopeSelector) {
            var cssText = "";
            return cssRules && Array.prototype.forEach.call(cssRules, function(rule) {
                if (rule.selectorText && rule.style && void 0 !== rule.style.cssText) cssText += this.scopeSelector(rule.selectorText, scopeSelector, this.strictStyling) + " {\n	", 
                ...
                cssText += this.scopeRules(rule.cssRules, scopeSelector), cssText += "\n}\n\n"; else try {
                    ...
                    rule.type === CSSRule.KEYFRAMES_RULE && rule.cssRules && (cssText += this.ieSafeCssTextFromKeyFrameRule(rule));
                }
            }, this), cssText;
        },
        ieSafeCssTextFromKeyFrameRule: function(rule) {
            var cssText = "@keyframes " + rule.name + " {";
            return Array.prototype.forEach.call(rule.cssRules, function(rule) {
        ...
        scopeSelector: function(selector, scopeSelector, strict) {
            ...
            return parts.forEach(function(p) {
                p = p.trim(), this.selectorNeedsScoping(p, scopeSelector) && (p = strict && !p.match(polyfillHostNoCombinator) ? this.applyStrictSelectorScope(p, scopeSelector) : this.applySelectorScope(p, scopeSelector)), 
                r.push(p);
            }, this), r.join(", ");
        },
        selectorNeedsScoping: function(selector, scopeSelector) {
            if (Array.isArray(scopeSelector)) return !0;
            var re = this.makeScopeMatcher(scopeSelector);
            return !selector.match(re);
        },
        makeScopeMatcher: function(scopeSelector) {
            return scopeSelector = scopeSelector.replace(/\[/g, "\\[").replace(/\]/g, "\\]"), 
        ...
        applySelectorScopeList: function(selector, scopeSelectorList) {
            for (var s, r = [], i = 0; s = scopeSelectorList[i]; i++) r.push(this.applySimpleSelectorScope(selector, s));
            return r.join(", ");
        },
        applySimpleSelectorScope: function(selector, scopeSelector) {
            return selector.match(polyfillHostRe) ? (selector = selector.replace(polyfillHostNoCombinator, scopeSelector), 
            selector.replace(polyfillHostRe, scopeSelector + " ")) : scopeSelector + " " + selector;
        },
        ...
        propertiesFromRule: function(rule) {
            ...
            rule.style.content && !rule.style.content.match(/['"]+|attr/) && (cssText = cssText.replace(/content:[^;]*;/g, "content: '" + rule.style.content + "';"));
            var style = rule.style;
            for (var i in style) "initial" === style[i] && (cssText += i + ": initial; ");
            return cssText;
        },
        replaceTextInStyles: function(styles, action) {
            styles && action && (styles instanceof Array || (styles = [ styles ]), Array.prototype.forEach.call(styles, function(s) {
                s.textContent = action.call(this, s.textContent);
            }, this));
        },
        addCssToDocument: function(cssText, name) {
            cssText.match("@import") ? addOwnSheet(cssText, name) : addCssToDocument(cssText);
        }
    }, cssCommentRe = /\/\*[^*]*\*+([^/*][^*]*\*+)*\//gim, cssCommentNextSelectorRe = /\/\*\s*@polyfill ([^*]*\*+([^/*][^*]*\*+)*\/)([^{]*?){/gim, cssContentNextSelectorRe = /polyfill-next-selector[^}]*content\:[\s]*?['"](.*?)['"][;\s]*}([^{]*?){/gim, cssCommentRuleRe = /\/\*\s@polyfill-rule([^*]*\*+([^/*][^*]*\*+)*)\//gim, cssContentRuleRe = /(polyfill-rule)[^}]*(content\:[\s]*['"](.*?)['"])[;\s]*[^}]*}/gim, cssCommentUnscopedRuleRe = /\/\*\s@polyfill-unscoped-rule([^*]*\*+([^/*][^*]*\*+)*)\//gim, cssContentUnscopedRuleRe = /(polyfill-unscoped-rule)[^}]*(content\:[\s]*['"](.*?)['"])[;\s]*[^}]*}/gim, polyfillHost = "-shadowcsshost", polyfillHostContext = "-shadowcsscontext", parenSuffix = ")(?:\\(((?:\\([^)(]*\\)|[^)(]*)+?)\\))?([^,{]*)", cssColonHostRe = new RegExp("(" + polyfillHost + parenSuffix, "gim"), cssColonHostContextRe = new RegExp("(" + polyfillHostContext + parenSuffix, "gim"), selectorReSuffix = "([>\\s~+[.,{:][\\s\\S]*)?$", colonHostRe = /\:host/gim, colonHostContextRe = /\:host-context/gim, polyfillHostNoCombinator = polyfillHost + "-no-combinator", polyfillHostRe = new RegExp(polyfillHost, "gim"), shadowDOMSelectorsRe = (new RegExp(polyfillHostContext, "gim"), 
    [ /\^\^/g, /\^/g, /\/shadow\//g, /\/shadow-deep\//g, /::shadow/g, /\/deep\//g, /::content/g ]), frame = document.createElement("iframe");
    frame.style.display = "none";
    var sheet, isChrome = navigator.userAgent.match("Chrome"), SHIM_ATTRIBUTE = "shim-shadowdom", SHIMMED_ATTRIBUTE = "shim-shadowdom-css", NO_SHIM_ATTRIBUTE = "no-shim";
    if (window.ShadowDOMPolyfill) {
        addCssToDocument("style { display: none !important; }\n");
        var doc = ShadowDOMPolyfill.wrap(document), head = doc.querySelector("head");
        head.insertBefore(getSheet(), head.childNodes[0]), document.addEventListener("DOMContentLoaded", function() {
            scope.urlResolver;
            if (window.HTMLImports && !HTMLImports.useNative) {
                var SHIM_SHEET_SELECTOR = "link[rel=stylesheet][" + SHIM_ATTRIBUTE + "]", SHIM_STYLE_SELECTOR = "style[" + SHIM_ATTRIBUTE + "]";
                HTMLImports.importer.documentPreloadSelectors += "," + SHIM_SHEET_SELECTOR, HTMLImports.importer.importsPreloadSelectors += "," + SHIM_SHEET_SELECTOR, 
                HTMLImports.parser.documentSelectors = [ HTMLImports.parser.documentSelectors, SHIM_SHEET_SELECTOR, SHIM_STYLE_SELECTOR ].join(",");
                var originalParseGeneric = HTMLImports.parser.parseGeneric;
                HTMLImports.parser.parseGeneric = function(elt) {
                    if (!elt[SHIMMED_ATTRIBUTE]) {
                        var style = elt.__importElement || elt;
                        if (!style.hasAttribute(SHIM_ATTRIBUTE)) return void originalParseGeneric.call(this, elt);
                        elt.__resource && (style = elt.ownerDocument.createElement("style"), style.textContent = elt.__resource), 
                        HTMLImports.path.resolveUrlsInStyle(style), style.textContent = ShadowCSS.shimStyle(style), 
                        style.removeAttribute(SHIM_ATTRIBUTE, ""), style.setAttribute(SHIMMED_ATTRIBUTE, ""), 
                        style[SHIMMED_ATTRIBUTE] = !0, style.parentNode !== head && (elt.parentNode === head ? head.replaceChild(style, elt) : this.addElementToDocument(style)), 
                        style.__importParsed = !0, this.markParsingComplete(elt), this.parseNext();
                    }
                };
                var hasResource = HTMLImports.parser.hasResource;
                HTMLImports.parser.hasResource = function(node) {
                    return "link" === node.localName && "stylesheet" === node.rel && node.hasAttribute(SHIM_ATTRIBUTE) ? node.__resource : hasResource.call(this, node);
                };
            }
        });
    }
    ...
    window.ShadowDOMPolyfill ? (window.wrap = ShadowDOMPolyfill.wrapIfNeeded, window.unwrap = ShadowDOMPolyfill.unwrapIfNeeded) : window.wrap = window.unwrap = function(n) {
        return n;
    };
    ...
    function whenReady(callback, doc) {
        doc = doc || rootDocument, whenDocumentReady(function() {
            watchImportsLoad(callback, doc);
        }, doc);
    }
    function isDocumentReady(doc) {
        return "complete" === doc.readyState || doc.readyState === requiredReadyState;
    }
    function whenDocumentReady(callback, doc) {
        if (isDocumentReady(doc)) callback && callback(); else {
            var checkReady = function() {
                ("complete" === doc.readyState || doc.readyState === requiredReadyState) && (doc.removeEventListener(READY_EVENT, checkReady), 
    ...
    function markTargetLoaded(event) {
        event.target.__loaded = !0;
    }
    function watchImportsLoad(callback, doc) {
        function checkDone() {
            loaded == l && callback && callback();
        }
        function loadedImport(e) {
            markTargetLoaded(e), loaded++, checkDone();
        }
        var imports = doc.querySelectorAll("link[rel=import]"), loaded = 0, l = imports.length;
        if (l) for (var imp, i = 0; l > i && (imp = imports[i]); i++) isImportLoaded(imp) ? loadedImport.call(imp, {
            target: imp
        }) : (imp.addEventListener("load", loadedImport), imp.addEventListener("error", loadedImport)); else checkDone();
    }
    function isImportLoaded(link) {
        return useNative ? link.__loaded || link["import"] && "loading" !== link["import"].readyState : link.__importParsed;
    }
    function handleImports(nodes) {
        for (var n, i = 0, l = nodes.length; l > i && (n = nodes[i]); i++) isImport(n) && handleImport(n);
    }
    function isImport(element) {
        return "link" === element.localName && "import" === element.rel;
    }
    function handleImport(element) {
        var loaded = element["import"];
        loaded ? markTargetLoaded({
            target: element
        }) : (element.addEventListener("load", markTargetLoaded), element.addEventListener("error", markTargetLoaded));
    }
    var IMPORT_LINK_TYPE = "import", useNative = Boolean(IMPORT_LINK_TYPE in document.createElement("link")), hasShadowDOMPolyfill = Boolean(window.ShadowDOMPolyfill), wrap = function(node) {
        return hasShadowDOMPolyfill ? ShadowDOMPolyfill.wrapIfNeeded(node) : node;
    }, rootDocument = wrap(document), currentScriptDescriptor = {
        get: function() {
            var script = HTMLImports.currentScript || document.currentScript || ("complete" !== document.readyState ? document.scripts[document.scripts.length - 1] : null);
            return wrap(script);
        },
        configurable: !0
    };
    Object.defineProperty(document, "_currentScript", currentScriptDescriptor), Object.defineProperty(rootDocument, "_currentScript", currentScriptDescriptor);
    var isIE = /Trident|Edge/.test(navigator.userAgent), requiredReadyState = isIE ? "complete" : "interactive", READY_EVENT = "readystatechange";
    useNative && (new MutationObserver(function(mxns) {
        for (var m, i = 0, l = mxns.length; l > i && (m = mxns[i]); i++) m.addedNodes && handleImports(m.addedNodes);
    }).observe(document.head, {
        childList: !0
    }), function() {
        if ("loading" === document.readyState) for (var imp, imports = document.querySelectorAll("link[rel=import]"), i = 0, l = imports.length; l > i && (imp = imports[i]); i++) handleImport(imp);
    }()), whenReady(function() {
        HTMLImports.ready = !0, HTMLImports.readyTime = new Date().getTime();
        var evt = rootDocument.createEvent("CustomEvent");
        evt.initCustomEvent("HTMLImportsLoaded", !0, !0, {}), rootDocument.dispatchEvent(evt);
    }), scope.IMPORT_LINK_TYPE = IMPORT_LINK_TYPE, scope.useNative = useNative, scope.rootDocument = rootDocument, 
    scope.whenReady = whenReady, scope.isIE = isIE;
}(HTMLImports), function(scope) {
    var modules = [], addModule = function(module) {
        modules.push(module);
    ...
    scope.addModule = addModule, scope.initializeModules = initializeModules;
}(HTMLImports), HTMLImports.addModule(function(scope) {
    var CSS_URL_REGEXP = /(url\()([^)]*)(\))/g, CSS_IMPORT_REGEXP = /(@import[\s]+(?!url\())([^;]*)(;)/g, path = {
        resolveUrlsInStyle: function(style) {
            var doc = style.ownerDocument, resolver = doc.createElement("a");
            return style.textContent = this.resolveUrlsInCssText(style.textContent, resolver), 
            style;
        },
        resolveUrlsInCssText: function(cssText, urlObj) {
            var r = this.replaceUrls(cssText, urlObj, CSS_URL_REGEXP);
            return r = this.replaceUrls(r, urlObj, CSS_IMPORT_REGEXP);
        },
        replaceUrls: function(text, urlObj, regexp) {
            return text.replace(regexp, function(m, pre, url, post) {
                var urlPath = url.replace(/["']/g, "");
                return urlObj.href = urlPath, urlPath = urlObj.href, pre + "'" + urlPath + "'" + post;
            });
        }
    };
    scope.path = path;
    ...
    var xhr = {
        async: !0,
        ok: function(request) {
            return request.status >= 200 && request.status < 300 || 304 === request.status || 0 === request.status;
        },
        load: function(url, next, nextContext) {
            ...
            request.open("GET", url, xhr.async), request.addEventListener("readystatechange", function() {
                if (4 === request.readyState) {
                    var locationHeader = request.getResponseHeader("Location"), redirectedUrl = null;
                    if (locationHeader) var redirectedUrl = "/" === locationHeader.substr(0, 1) ? location.origin + locationHeader : locationHeader;
    ...
    var xhr = scope.xhr, flags = scope.flags, Loader = function(onLoad, onComplete) {
        this.cache = {}, this.onload = onLoad, this.oncomplete = onComplete, this.inflight = 0, 
        this.pending = {};
    };
    Loader.prototype = {
        addNodes: function(nodes) {
            this.inflight += nodes.length;
            for (var n, i = 0, l = nodes.length; l > i && (n = nodes[i]); i++) this.require(n);
            this.checkDone();
        },
        addNode: function(node) {
            this.inflight++, this.require(node), this.checkDone();
        },
        require: function(elt) {
            var url = elt.src || elt.href;
            elt.__nodeUrl = url, this.dedupe(url, elt) || this.fetch(url, elt);
        },
        dedupe: function(url, elt) {
            if (this.pending[url]) return this.pending[url].push(elt), !0;
            return this.cache[url] ? (this.onload(url, elt, this.cache[url]), this.tail(), !0) : (this.pending[url] = [ elt ], 
            !1);
        },
        fetch: function(url, elt) {
            if (flags.load && console.log("fetch", url, elt), url) if (url.match(/^data:/)) {
                var pieces = url.split(","), header = pieces[0], body = pieces[1];
                body = header.indexOf(";base64") > -1 ? atob(body) : decodeURIComponent(body), setTimeout(function() {
        ...
        receive: function(url, elt, err, resource, redirectedUrl) {
            ...
            for (var p, $p = this.pending[url], i = 0, l = $p.length; l > i && (p = $p[i]); i++) this.onload(url, p, resource, err, redirectedUrl), 
            this.tail();
    ...
    var Observer = function(addCallback) {
        this.addCallback = addCallback, this.mo = new MutationObserver(this.handler.bind(this));
    };
    Observer.prototype = {
        handler: function(mutations) {
            for (var m, i = 0, l = mutations.length; l > i && (m = mutations[i]); i++) "childList" === m.type && m.addedNodes.length && this.addedNodes(m.addedNodes);
        },
        addedNodes: function(nodes) {
            this.addCallback && this.addCallback(nodes);
            for (var n, i = 0, l = nodes.length; l > i && (n = nodes[i]); i++) n.children && n.children.length && this.addedNodes(n.children);
        },
    ...
    function nodeIsImport(elt) {
        return "link" === elt.localName && elt.rel === IMPORT_LINK_TYPE;
    }
    ...
    function generateSourceMapHint(script) {
        ...
        var moniker = script.ownerDocument.baseURI, num = owner.__importedScripts ? "-" + owner.__importedScripts : "";
        return owner.__importedScripts++, "\n//# sourceURL=" + moniker + num + ".js\n";
    }
    function cloneStyle(style) {
        var clone = style.ownerDocument.createElement("style");
        return clone.textContent = style.textContent, path.resolveUrlsInStyle(clone), clone;
    }
    var path = scope.path, rootDocument = scope.rootDocument, flags = scope.flags, isIE = scope.isIE, IMPORT_LINK_TYPE = scope.IMPORT_LINK_TYPE, IMPORT_SELECTOR = "link[rel=" + IMPORT_LINK_TYPE + "]", importParser = {
        ...
        parse: function(elt) {
            if (this.isParsed(elt)) return void (flags.parse && console.log("[%s] is already parsed", elt.localName));
            var fn = this[this.map[elt.localName]];
            fn && (this.markParsing(elt), fn.call(this, elt));
        },
        parseDynamic: function(elt, quiet) {
            this.dynamicElements.push(elt), quiet || this.parseNext();
        },
        markParsing: function(elt) {
            flags.parse && console.log("parsing", elt), this.parsingElement = elt;
        },
        markParsingComplete: function(elt) {
            elt.__importParsed = !0, this.markDynamicParsingComplete(elt), elt.__importElement && (elt.__importElement.__importParsed = !0, 
            this.markDynamicParsingComplete(elt.__importElement)), this.parsingElement = null, 
            flags.parse && console.log("completed", elt);
        },
        markDynamicParsingComplete: function(elt) {
            var i = this.dynamicElements.indexOf(elt);
            i >= 0 && this.dynamicElements.splice(i, 1);
        },
        parseImport: function(elt) {
            if (HTMLImports.__importsParsingHook && HTMLImports.__importsParsingHook(elt), elt["import"] && (elt["import"].__importParsed = !0), 
            this.markParsingComplete(elt), elt.dispatchEvent(elt.__resource && !elt.__error ? new CustomEvent("load", {
                bubbles: !1
            }) : new CustomEvent("error", {
                bubbles: !1
            })), elt.__pending) for (var fn; elt.__pending.length; ) fn = elt.__pending.shift(), 
            fn && fn({
                target: elt
            });
            this.parseNext();
        ...
        parseLink: function(linkElt) {
            nodeIsImport(linkElt) ? this.parseImport(linkElt) : (linkElt.href = linkElt.href, 
            this.parseGeneric(linkElt));
        },
        parseStyle: function(elt) {
            var src = elt;
            elt = cloneStyle(elt), elt.__importElement = src, this.parseGeneric(elt);
        },
        parseGeneric: function(elt) {
            this.trackElement(elt), this.addElementToDocument(elt);
        },
        rootImportForElement: function(elt) {
            for (var n = elt; n.ownerDocument.__importLink; ) n = n.ownerDocument.__importLink;
            return n;
        },
        ...
        trackElement: function(elt, callback) {
            var self = this, done = function(e) {
                callback && callback(e), self.markParsingComplete(elt), self.parseNext();
            };
            if (elt.addEventListener("load", done), elt.addEventListener("error", done), isIE && "style" === elt.localName) {
                var fakeLoad = !1;
                if (-1 == elt.textContent.indexOf("@import")) fakeLoad = !0; else if (elt.sheet) {
                    fakeLoad = !0;
                    for (var r, csr = elt.sheet.cssRules, len = csr ? csr.length : 0, i = 0; len > i && (r = csr[i]); i++) r.type === CSSRule.IMPORT_RULE && (fakeLoad = fakeLoad && Boolean(r.styleSheet));
                }
                fakeLoad && elt.dispatchEvent(new CustomEvent("load", {
                    bubbles: !1
                }));
            }
        },
        parseScript: function(scriptElt) {
            ...
            script.__importElement = scriptElt, script.src = scriptElt.src ? scriptElt.src : generateScriptDataUrl(scriptElt), 
            scope.currentScript = scriptElt, this.trackElement(script, function() {
                script.parentNode.removeChild(script), scope.currentScript = null;
            }), this.addElementToDocument(script);
        },
        nextToParse: function() {
            return this._mayParse = [], !this.parsingElement && (this.nextToParseInDoc(rootDocument) || this.nextToParseDynamic());
        },
        nextToParseInDoc: function(doc, link) {
            if (doc && this._mayParse.indexOf(doc) < 0) {
                this._mayParse.push(doc);
                for (var n, nodes = doc.querySelectorAll(this.parseSelectorsForNode(doc)), i = 0, l = nodes.length; l > i && (n = nodes[i]); i++) if (!this.isParsed(n)) return this.hasResource(n) ? nodeIsImport(n) ? this.nextToParseInDoc(n["import"], n) : n : void 0;
            }
            return link;
        },
        ...
        hasResource: function(node) {
            return nodeIsImport(node) && void 0 === node["import"] ? !1 : !0;
        }
    };
    scope.parser = importParser, scope.IMPORT_SELECTOR = IMPORT_SELECTOR;
}), HTMLImports.addModule(function(scope) {
    ...
    function makeDocument(resource, url) {
        ...
        base.setAttribute("href", url), doc.baseURI || hasBaseURIAccessor(doc) || Object.defineProperty(doc, "baseURI", {
            value: url
        });
        var meta = doc.createElement("meta");
        return meta.setAttribute("charset", "utf-8"), doc.head.appendChild(meta), doc.head.appendChild(base), 
        doc.body.innerHTML = resource, window.HTMLTemplateElement && HTMLTemplateElement.bootstrap && HTMLTemplateElement.bootstrap(doc), 
        doc;
    }
    var flags = scope.flags, IMPORT_LINK_TYPE = scope.IMPORT_LINK_TYPE, IMPORT_SELECTOR = scope.IMPORT_SELECTOR, rootDocument = scope.rootDocument, Loader = scope.Loader, Observer = scope.Observer, parser = scope.parser, importer = {
        documents: {},
        ...
        loaded: function(url, elt, resource, err, redirectedUrl) {
            if (flags.load && console.log("loaded", url, elt), elt.__resource = resource, elt.__error = err, 
            isImportLink(elt)) {
                var doc = this.documents[url];
                void 0 === doc && (doc = err ? null : makeDocument(resource, redirectedUrl || url), 
                doc && (doc.__importLink = elt, this.bootDocument(doc)), this.documents[url] = doc), 
                elt["import"] = doc;
            }
            parser.parseNext();
        },
        bootDocument: function(doc) {
            this.loadSubtree(doc), this.observer.observe(doc), parser.parseNext();
        },
        loadedAll: function() {
            parser.parseNext();
        }
    }, importLoader = new Loader(importer.loaded.bind(importer), importer.loadedAll.bind(importer));
    if (importer.observer = new Observer(), !document.baseURI) {
        ...
        Object.defineProperty(document, "baseURI", baseURIDescriptor), Object.defineProperty(rootDocument, "baseURI", baseURIDescriptor);
    }
    scope.importer = importer, scope.importLoader = importLoader;
}), HTMLImports.addModule(function(scope) {
    var parser = scope.parser, importer = scope.importer, dynamic = {
        added: function(nodes) {
            for (var owner, parsed, loading, n, i = 0, l = nodes.length; l > i && (n = nodes[i]); i++) owner || (owner = n.ownerDocument, 
            parsed = parser.isParsed(owner)), loading = this.shouldLoadNode(n), loading && importer.loadNode(n), 
            this.shouldParseNode(n) && parsed && parser.parseDynamic(n, loading);
        },
        shouldLoadNode: function(node) {
            return 1 === node.nodeType && matches.call(node, importer.loadSelectorsForNode(node));
        },
        shouldParseNode: function(node) {
            return 1 === node.nodeType && matches.call(node, parser.parseSelectorsForNode(node));
        }
    };
    ...
    var initializeModules = scope.initializeModules, isIE = scope.isIE;
    if (!scope.useNative) {
        isIE && "function" != typeof window.CustomEvent && (window.CustomEvent = function(inType, params) {
            params = params || {};
            var e = document.createEvent("CustomEvent");
            return e.initCustomEvent(inType, Boolean(params.bubbles), Boolean(params.cancelable), params.detail), 
            e;
        }, window.CustomEvent.prototype = window.Event.prototype), initializeModules();
        var rootDocument = scope.rootDocument;
        "complete" === document.readyState || "interactive" === document.readyState && !window.attachEvent ? bootstrap() : document.addEventListener("DOMContentLoaded", bootstrap);
    }
}(HTMLImports), window.CustomElements = window.CustomElements || {
    flags: {}
}, function(scope) {
    var flags = scope.flags, modules = [], addModule = function(module) {
        modules.push(module);
    }, initializeModules = function() {
        modules.forEach(function(module) {
            module(scope);
    ...
    scope.addModule = addModule, scope.initializeModules = initializeModules, scope.hasNative = Boolean(document.registerElement), 
    scope.useNative = !flags.register && scope.hasNative && !window.ShadowDOMPolyfill && (!window.HTMLImports || HTMLImports.useNative);
}(CustomElements), CustomElements.addModule(function(scope) {
    ...
    function forDocumentTree(doc, cb) {
        processingDocuments = [], _forDocumentTree(doc, cb), processingDocuments = null;
    }
    function _forDocumentTree(doc, cb) {
        if (doc = wrap(doc), !(processingDocuments.indexOf(doc) >= 0)) {
            processingDocuments.push(doc);
            for (var n, imports = doc.querySelectorAll("link[rel=" + IMPORT_LINK_TYPE + "]"), i = 0, l = imports.length; l > i && (n = imports[i]); i++) n["import"] && _forDocumentTree(n["import"], cb);
            cb(doc);
        }
    }
    var processingDocuments, IMPORT_LINK_TYPE = window.HTMLImports ? HTMLImports.IMPORT_LINK_TYPE : "none";
    ...
    function _attached(element) {
        element.__upgraded__ && (element.attachedCallback || element.detachedCallback) && !element.__attached && inDocument(element) && (element.__attached = !0, 
    ...
    function _detached(element) {
        element.__upgraded__ && (element.attachedCallback || element.detachedCallback) && element.__attached && !inDocument(element) && (element.__attached = !1, 
        element.detachedCallback && element.detachedCallback());
    }
    ...
    function upgradeDocument(doc) {
        doc = wrap(doc), flags.dom && console.group("upgradeDocument: ", doc.baseURI.split("/").pop()), 
        addedNode(doc), observe(doc), flags.dom && console.groupEnd();
    }
    function upgradeDocumentTree(doc) {
        forDocumentTree(doc, upgradeDocument);
    }
    var flags = scope.flags, forSubtree = scope.forSubtree, forDocumentTree = scope.forDocumentTree, hasPolyfillMutations = !window.MutationObserver || window.MutationObserver === window.JsMutationObserver;
    scope.hasPolyfillMutations = hasPolyfillMutations;
    var isPendingMutations = !1, pendingMutations = [], forEach = Array.prototype.forEach.call.bind(Array.prototype.forEach), originalCreateShadowRoot = Element.prototype.createShadowRoot;
    originalCreateShadowRoot && (Element.prototype.createShadowRoot = function() {
        var root = originalCreateShadowRoot.call(this);
        return CustomElements.watchShadow(this), root;
    }), scope.watchShadow = watchShadow, scope.upgradeDocumentTree = upgradeDocumentTree, 
    scope.upgradeSubtree = addedSubtree, scope.upgradeAll = addedNode, scope.attachedNode = attachedNode, 
    scope.takeRecords = takeRecords;
}), CustomElements.addModule(function(scope) {
    function upgrade(node) {
        if (!node.__upgraded__ && node.nodeType === Node.ELEMENT_NODE) {
            var is = node.getAttribute("is"), definition = scope.getRegisteredDefinition(is || node.localName);
            if (definition) {
    ...
    function upgradeWithDefinition(element, definition) {
        return flags.upgrade && console.group("upgrade:", element.localName), definition.is && element.setAttribute("is", definition.is), 
        implementPrototype(element, definition), element.__upgraded__ = !0, created(element), 
        scope.attachedNode(element), scope.upgradeSubtree(element), flags.upgrade && console.groupEnd(), 
        element;
    }
    function implementPrototype(element, definition) {
        Object.__proto__ ? element.__proto__ = definition.prototype : (customMixin(element, definition.prototype, definition["native"]), 
    ...
    function register(name, options) {
        ...
        return definition.prototype || (definition.prototype = Object.create(HTMLElement.prototype)), 
        definition.__name = name.toLowerCase(), definition.lifecycle = definition.lifecycle || {}, 
        definition.ancestry = ancestry(definition["extends"]), resolveTagName(definition), 
        resolvePrototypeChain(definition), overrideAttributeApi(definition.prototype), registerDefinition(definition.__name, definition), 
        definition.ctor = generateConstructor(definition), definition.ctor.prototype = definition.prototype, 
        definition.prototype.constructor = definition.ctor, scope.ready && upgradeDocumentTree(document), 
        definition.ctor;
    }
    ...
    function isReservedTag(name) {
        for (var i = 0; i < reservedTagList.length; i++) if (name === reservedTagList[i]) return !0;
    }
    ...
    function resolvePrototypeChain(definition) {
        if (!Object.__proto__) {
            ...
            if (definition.is) {
                var inst = document.createElement(definition.tag), expectedPrototype = Object.getPrototypeOf(inst);
                expectedPrototype === definition.prototype && (nativePrototype = expectedPrototype);
            }
            for (var ancestor, proto = definition.prototype; proto && proto !== nativePrototype; ) ancestor = Object.getPrototypeOf(proto), 
    ...
    function createElement(tag, typeExtension) {
        ...
        var element;
        return typeExtension ? (element = createElement(tag), element.setAttribute("is", typeExtension), 
        element) : (element = domCreateElement(tag), tag.indexOf("-") >= 0 && implementPrototype(element, HTMLElement), 
        element);
    }
    ...
    var isInstance, upgradeDocumentTree = scope.upgradeDocumentTree, upgrade = scope.upgrade, upgradeWithDefinition = scope.upgradeWithDefinition, implementPrototype = scope.implementPrototype, useNative = scope.useNative, reservedTagList = [ "annotation-xml", "color-profile", "font-face", "font-face-src", "font-face-uri", "font-face-format", "font-face-name", "missing-glyph" ], registry = {}, HTML_NAMESPACE = "http://www.w3.org/1999/xhtml", domCreateElement = document.createElement.bind(document), domCreateElementNS = document.createElementNS.bind(document), domCloneNode = Node.prototype.cloneNode;
    isInstance = Object.__proto__ || useNative ? function(obj, base) {
        ...
        return !1;
    }, document.registerElement = register, document.createElement = createElement, 
    document.createElementNS = createElementNS, Node.prototype.cloneNode = cloneNode, 
    scope.registry = registry, scope["instanceof"] = isInstance, scope.reservedTagList = reservedTagList, 
    scope.getRegisteredDefinition = getRegisteredDefinition, document.register = document.registerElement;
}), function(scope) {
    function bootstrap() {
    ...
    var useNative = scope.useNative, initializeModules = scope.initializeModules, isIE11OrOlder = /Trident/.test(navigator.userAgent);
    if (useNative) {
        var nop = function() {};
        scope.watchShadow = nop, scope.upgrade = nop, scope.upgradeAll = nop, scope.upgradeDocumentTree = nop, 
        scope.upgradeSubtree = nop, scope.takeRecords = nop, scope["instanceof"] = function(obj, base) {
            return obj instanceof base;
        };
    } else initializeModules();
    var upgradeDocumentTree = scope.upgradeDocumentTree;
    if (window.wrap || (window.ShadowDOMPolyfill ? (window.wrap = ShadowDOMPolyfill.wrapIfNeeded, 
    window.unwrap = ShadowDOMPolyfill.unwrapIfNeeded) : window.wrap = window.unwrap = function(node) {
        return node;
        ...
        return e.initCustomEvent(inType, Boolean(params.bubbles), Boolean(params.cancelable), params.detail), 
        e;
    }, window.CustomEvent.prototype = window.Event.prototype), "complete" === document.readyState || scope.flags.eager) bootstrap(); else if ("interactive" !== document.readyState || window.attachEvent || window.HTMLImports && !window.HTMLImports.ready) {
    ...
    function installPolymerWarning() {
        window.Polymer === polymerStub && (window.Polymer = function() {
    ...
    var elementDeclarations = [], polymerStub = function(name) {
        "string" != typeof name && 1 === arguments.length && Array.prototype.push.call(arguments, document._currentScript), 
        elementDeclarations.push(arguments);
    };
    window.Polymer = polymerStub, scope.consumeDeclarations = function(callback) {
        scope.consumeDeclarations = function() {
            throw "Possible attempt to load Polymer twice";
        }, callback && callback(elementDeclarations), elementDeclarations = null;
    }, HTMLImports.useNative ? installPolymerWarning() : addEventListener("DOMContentLoaded", installPolymerWarning);
}(window.WebComponents), function() {
    var style = document.createElement("style");
