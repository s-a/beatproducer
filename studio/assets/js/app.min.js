...
for(var l in b)d.setRequestHeader(l,b[l]);d.responseType=h,d.send(i)},abort:function(){c.abort()}}:void 0});var i=function(){return this.say=function(a,b){if(window.SpeechSynthesisUtterance&&window.speechSynthesis){var c=new SpeechSynthesisUtterance;
//msg.voice = voices[1]; // Note: some voices don't support altering params
c.voiceURI="native",c.volume=1,// 0 to 1
// msg.rate = 0; // 0.1 to 10
// msg.pitch = 1; //0 to 2
c.text=a,c.lang="en-US";var d=!1;c.onend=function(){d=!0,console.log("DONE!"),b&&b()},window.speechSynthesis.speak(c),window.setTimeout(function(){d||b&&b()},2e3)}else b&&b()},this},j=function(b){b&&(this._el=b),this.audioContext=e,this.bufSrc=null,
//  7  this._chored = false;
//    this._db = {};
this._comp=this.audioContext.createDynamicsCompressor(),this._comp.connect(this.audioContext.destination),this._comp.threshold.value=-18,this._comp.knee.value=8,this.destination=this._comp,this.transportTime=0,this._loadingProgressIndicatorInterval=null,this.markers=[],this._regions=[];var c=this;return a(window).resize(function(){c.onResize()}),this};j.prototype.addMarker=function(a){var b=function(a,b){return a-b};if(a){a=a.sort(b);for(var c=0;c<a.length;c++){var d=a[c],e=this.spectrumPosition({seconds:d});this.markers.push(e)}this.computeSpectrumPositions(),this.refreshSpectrum()}},j.prototype.onResize=function(){var a=this;window.clearTimeout(this.___to),this.___to=window.setTimeout(function(){a.computeSpectrumPositions(),a.initializeSpectrum()},400)},j.prototype.$=function(){return a(this._el.getDOMNode())},j.prototype.get=function(b,c){{var d=this;a.ajax({url:b,type:"GET",dataType:"binary",responseType:"arraybuffer",processData:!1,success:function(b){c&&a.proxy(c,d)(b)}})}},j.prototype.load=function(b,d){var e=this,g=function(b){e.tape=new c.Tape(b),e.renderAudio(function(){e.setLoadingProgressIndicator(!1),e.initializeSpectrum(),d&&a.proxy(d,e)()})},h=function(a){e._el.setState({name:e.name}),e.audioContext.decodeAudioData(a,function(a){
//self.rawData = decodedData;
g(a)})};"string"==typeof b?(this.name||(this.name=b.split("/").pop()),this.filename||(this.filename=b.replace(f,"")),this.get(b,function(a){h(a)})):this.renderAudio(function(a){g(a)})},j.prototype.setLoadingProgressIndicator=function(a){if(a){this.$().find("canvas").remove();var c=this.$().find(".device-spectrum").get(0),d=[],e=new b({container:c,interpolate:!1}),f=e.context,g=f.createLinearGradient(0,0,0,e.height);g.addColorStop(0,"#f60"),g.addColorStop(1,"#ff1b00"),e.innerColor=g;var h=0;this._loadingProgressIndicatorInterval=setInterval(function(){h++,d.push(Math.cos(h/25)-.2+.3*Math.random()),e.update({data:d})},1)}else this.$().find("canvas").remove(),window.clearInterval(this._loadingProgressIndicatorInterval)},j.prototype.computeSpectrumPositions=function(){if(this.waveform)for(var b=a(this.waveform.container).width(),c=0;c<this.markers.length;c++){var d=this.markers[c];if(0!==d.percent&&!d.percent)throw"Need percantage value to compute slice positions in spectrum.";d.x=b/100*d.percent}},j.prototype.spectrumPosition=function(b){var c=this.audioBuffer.duration,d={};if(b.x&&(d.x=b.x,d.percent=d.x/a(this.waveform.container).width()*100,d.seconds=d.percent/100*c),0===b.seconds&&(d.x=0,d.percent=0,d.seconds=0),b.seconds){var e=a(this.waveform.container).width();d.percent=b.seconds/c*100,d.x=e/100*d.percent,d.seconds=b.seconds}return d},j.prototype.refreshSpectrum=function(a){this.waveform&&(a||this.waveform.redraw(),this.refreshSpectrumMarkers(),this.spectrumPositionX&&k(this.waveform.context,this.spectrumPositionX,"#f60"))};var k=function(a,b,c){a.beginPath(),a.moveTo(b,0),a.lineTo(b,190),a.strokeStyle=c,a.stroke()};j.prototype.spectrumCursorPosition=function(a){this.spectrumPositionX=a,this.refreshSpectrum()},j.prototype.refreshSpectrumMarkers=function(){for(var a=0;a<this.markers.length;a++){var b=this.markers[a];k(this.waveform.context,b.x,"#fff"),k(this.waveform.context,b.x,"#fff")}},j.prototype.initializeSpectrum=function(){this.$().find("canvas").remove();var a=this.$().find(".device-spectrum").get(0);if(a){var c=this.audioBuffer.getChannelData(1);this.waveform=new b({container:a,interpolate:!0,innerColor:function(a){var b=this.device.audioBuffer.duration,c=this.device.transportTime/1e3,d=c/b;
//console.log(i,d);
//console.log(i,d);
return d>a?"#f60":"#676F98"}}),this.waveform.device=this,this.waveform.update({data:c});var d=this;d.refreshSpectrum(!0)}},j.prototype.renderAudio=function(b){var c=this;this.tape.render(this.audioContext,2).then(function(d){c.audioBuffer=d,b&&a.proxy(b,c)(d)})},j.prototype.onPlayBack=function(a){a||(this.transportTime+=h),this.refreshSpectrum();var b=0;this.transportTime>0&&(b=this.transportTime/1e3),this._el.setState({currentTime:d(b,1)})},j.prototype.play=function(b){if(this.pause(),this.tape){var c=this;c.bufSrc=c.audioContext.createBufferSource(),c.bufSrc.buffer=c.audioBuffer,c.bufSrc.connect(c.destination),c.bufSrc.onended=function(){window.clearInterval(c.onPlayBackTimerInterval),a.proxy(b,c)};var d=a.proxy(c.onPlayBack,c);c.onPlayBackTimerInterval=window.setInterval(d,h),d();var e=0;this.transportTime>0&&(e=this.transportTime/1e3),c.bufSrc.start(0,e)}},j.prototype.stop=function(){var a=this;a.pause(),a.transportTime=0,a.onPlayBack(!0)},j.prototype.pause=function(){this.tape&&this.bufSrc&&(window.clearInterval(this.onPlayBackTimerInterval),this.bufSrc.stop())},j.prototype.open=function(b,c){var d=this;d._el?(d.load(b,function(){d.chopSlices(),c&&a.proxy(c,d)(d)}),d.setLoadingProgressIndicator(!0)):(d.parentElement||(d.parentElement=d.project._el.getDOMNode()),d.render(function(){d._el=this,d.setLoadingProgressIndicator(!0),d.load(b,function(){d.chopSlices(),c&&a.proxy(c,d)(d)})}))},j.prototype.slice=function(a){return this.slices(a)},j.prototype.chopSlices=function(){var a=function(a,b){return a.seconds-b.seconds};this.markers=this.markers.sort(a),this.slices=[];for(var b=0;b<this.markers.length-1;b++){var c=this.markers[b].seconds,d=this.markers[b+1].seconds-this.markers[b].seconds,e=this.tape.slice(c,d);this.slices.push(new p(e))}},j.prototype.config=function(){var a=this,b=function(){var b=[];if(a.markers)for(var c=0;c<a.markers.length;c++)b.push(a.markers[c].seconds);return b},c=function(){var b=[];if(a.patterns)for(var c=0;c<a.patterns.length;c++){var d=a.patterns[c];b.push({id:d.id,bars:d.bars,sequence:d.sequence})}return b};return{name:a.name,sample:this.filename,slices:b(),patterns:c()}};/* ******************** PROJECT ************************************************************  */
var l=function(a){return this.parentElement=a,this};l.prototype.$=function(){return a(this._el.getDOMNode())},l.prototype.reset=function(){this._el.setState({devices:[]})},l.prototype.newDevice=function(b,c){var d=this,e=new j,g=d._el.state.devices,h=function(){e.addMarker(b.slices),e.chopSlices(),c&&a.proxy(c,d)(e)};e.project=d,g.push(e),d._el.setState({devices:g},function(){if(b.sample){if(e.name=b.name,!e.name){var a=b.sample.split("/");a.length>1&&a.shift(),a=a.join("//"),e.name=a}e.filename=b.sample,e.open(f+b.sample,h)}e.patterns=[];for(var c=0;c<b.patterns.length;c++)e.patterns.push(new q(e,b.patterns[c]));b.tape&&(e.tape=b.tape,e.load(null,h))})},l.prototype.deviceContainer=function(a){return this.$().find(".project-device-container-"+a).get(0)},l.prototype.renderGUI=function(b){var c=this;c.render?c.render(function(){c._el=this,b&&a.proxy(b,c)()}):b()},l.prototype.open=function(a,b){var c=this;if(!a||!a.devices)return void b();c.all||(c.all=a.devices.length),c._el.setState({bpm:a.bpm,name:a.name});var d=function(){c.all--,0===c.all&&(c.all=null,b(a))};a.devices&&0!==a.devices.length||b();for(var e=0;e<a.devices.length;e++)c.newDevice(a.devices[e],d)},l.prototype.config=function(){for(var a={name:this.name,bpm:this.bpm,devices:[]},b=0;b<this._el.state.devices.length;b++){var c=this._el.state.devices[b].config();c.sample&&a.devices.push(c)}return a},l.prototype.configString=function(){return JSON.stringify(this.config(),"	",1)};/* ******************** SEQUENCER ************************************************************  */
var m=function(){return this};m.prototype.$=function(){return a(this._el.getDOMNode())},m.prototype.renderGUI=function(b){var c=this;c.render(function(){c._el=this,b&&a.proxy(b,c)()})};/* ******************** PATTERN-EDITOR ************************************************************  */
var n=function(){return this};n.prototype.$=function(){return a(this._el.getDOMNode())},n.prototype.renderGUI=function(b){var c=this;c.render?c.render(function(){c._el=this,b&&a.proxy(b,c)()}):b()},n.prototype.open=function(a){this._el.setState({connectedDevice:a,connectedDeviceName:a.name})};/* ******************** STUDIO ************************************************************  */
var o=function(){return this};l.prototype.$=function(){return a(this._el.getDOMNode())},o.prototype.Speech=i,o.prototype.Project=l,o.prototype.renderGUI=function(a,b){var c=this;c.project=new c.Project(a),c.project.studio=c,c.project.renderGUI(function(){c.patternEditor=new n,c.patternEditor.renderGUI(function(){
//$('#gui').fadeIn("slow", function(){
b()})})},o.prototype.reset=function(){this.project&&this.project.reset()},o.prototype.init=function(b,c){var d=this;d.reset();(new this.Speech).say;b&&(document.title=b.name),b&&d.gui.bpmSlider&&(d.gui.bpmSlider.value=b.bpm),a("#gui-loading-progress").fadeIn("fast",function(){}),
//$(function() {
...
var p=function(a){var b=1;
//  7  this._chored = false;
//    this._db = {};
return a=c.concat(a.split(1e3).map(function(a,c){var d=.1;return c>990&&(b-=d),a.gain(b)})),this.tape=a,this.audioContext=e,this.bufSrc=null,this._comp=this.audioContext.createDynamicsCompressor(),this._comp.connect(this.audioContext.destination),this._comp.threshold.value=-18,this._comp.knee.value=8,this.destination=this._comp,this};p.prototype.play=function(b){if(this.tape){var c=this;this.tape.render(this.audioContext,2).then(function(d){c.audioBuffer=d,c.bufSrc=c.audioContext.createBufferSource(),c.bufSrc.buffer=c.audioBuffer,c.bufSrc.connect(c.destination),c.bufSrc.onended=function(){b&&a.proxy(b,c)},c.bufSrc.start(0,0)})}};/* ******************** PATTERN ************************************************************  */
var q=function(a,b){
//  7  this._chored = false;
//    this._db = {};
return this.device=a,this.id=b.id,this.bars=b.bars,this.sequence=b.sequence,this.patternConfig=b,this.audioContext=e,this.bufSrc=null,this._comp=this.audioContext.createDynamicsCompressor(),this._comp.connect(this.audioContext.destination),this._comp.threshold.value=-18,this._comp.knee.value=8,this.destination=this._comp,this};q.prototype.toString=function(a){for(var b=this.sequence,c="",d=0;d<b.length;d++){var e=b[d];c+=-1===e.indexOf(a)?" ":"a"}return c},q.prototype.renderSequence=function(a){for(var b,d=this.device.project.bpm,e=1,f=6e4/d/1e3/(2*e),g=0;g<this.device.slices.length;g++){var h=this.device.slices[g].tape,i=new c.Sequence(this.toString(g+1),f,{a:h}).apply();b=b?b.mix(i):i}this.tape=b,a(b)},q.prototype.play=function(b){var c=this;c.renderSequence(function(){c.tape.render(c.audioContext,2).then(function(d){c.audioBuffer=d,c.bufSrc=c.audioContext.createBufferSource(),c.bufSrc.buffer=c.audioBuffer,c.bufSrc.connect(c.destination),c.bufSrc.onended=function(){a.proxy(b,c)},c.bufSrc.loop=!0,c.bufSrc.start(0,0)})})},q.prototype.stop=function(){var a=this;a.pause(),a.transportTime=0},q.prototype.pause=function(){this.tape&&this.bufSrc&&this.bufSrc.stop()};/* ******************** USER ************************************************************  */
var r=function(){return this.mainRepoName="beatproducer-projects",this};r.prototype.login=function(a,b,c){this.github=new window.Octokit({username:a,password:b,auth:"basic"}),this.username=a,this.initialzeRepo(c)},r.prototype.forkRepo=function(a){var b=this;this.fork=this.github.getRepo(this.username,b.mainRepoName),this.repo=this.github.getRepo("s-a",b.mainRepoName);var c=function(c){401===c.status?b.onError(c):b.repo.fork().then(function(){window.setTimeout(function(){// give the fork some time
b.fork=b.github.getRepo(b.username,b.mainRepoName),a&&a()},5e3)},b.onError)};this.fork.getInfo().then(a,c)},r.prototype.initialzeRepo=function(a){this.forkRepo(a)},r.prototype.newSong=function(b,c){var d=this;a("#gui-loading-progress").fadeIn("fast",function(){d.branchname=b,d.branch=d.fork.getBranch("master"),d.branch.createBranch(b).then(function(){d.branch=d.fork.getBranch(b),c()},d.onError)})},r.prototype.saveSong=function(a,b,c){var d=this,e="projects/"+this.username+" "+this.branchname+".json";this.branch.write(e,a,b,!1).then(function(){window.gui.user=d,c&&c()},this.onError)},r.prototype.onError=function(b){var c=JSON.parse(b.message).errors;c?c=c[c.length-1]:(c=JSON.parse(b.message),c.resource="");var d=c.resource+"\n\n"+c.message;a("#gui-loading-progress").fadeOut("fast",function(){401===b.status&&window.gui.onLoginClick(),window.gui.alert(d)})},r.prototype.publish=function(b){var c=this,d={title:"I' d like to publish my new song. Yo!",body:"This pull request has been automatically generated by the beatproducer-GUI.",base:"master",head:this.username+":"+this.branchname};a("#gui-loading-progress").fadeIn("fast",function(){c.repo.createPullRequest(d).then(b,c.onError)})};/* ******************** GUI ************************************************************  */
var s=function(b){return g?(this.studio=b,this.credentials={uid:a.cookie("uid"),pwd:a.cookie("pwd")},a(".input-login-uid:first").val(this.credentials.uid),a(".input-login-pwd:first").val(this.credentials.pwd),this.initEventListeners()):(this.alert("Oh NO! Your Browser does not suppert AudioContext!?. Take a look at supported browser list to use this website. Maybe one day your browser will support new web technologies. o.O"),window.location="https://github.com/s-a/beatproducer#browser-support"),this};s.prototype.alert=function(b){var c=a('<paper-toast duration="5000" text="'+b+'"></paper-toast>');a("body").append(c),c.get(0).addEventListener("core-overlay-close-completed",function(){c.remove()}),c.get(0).show&&c.get(0).show()},s.prototype.onLoginClick=function(){document.querySelector("#login-dialog").toggle()},s.prototype.onOpenProjectClick=function(){var b=window.gui;if(!(b.user||window.gui.credentials.uid&&window.gui.credentials.pwd))return void(new r).onError({status:401,message:'{"errors":[{"resource": "New Project","message":"No user login"}]}'});var c=function(a){for(var b=JSON.parse(a),c=[],d=0;d<b.length;d++){var e=b[d],f=e.name.split(" "),g=f.pop().split(".json")[0],h=f.join(" ");c.push({user:h,name:g})}return c},d=function(a,b){var c=a.github.getRepo("s-a","beatproducer-projects"),d=c.getBranch("master");d.contents("projects").then(b,a.onError)};a("#gui-loading-progress").fadeIn("fast",function(){});var e=new window.boom.User;e.login(window.gui.credentials.uid,window.gui.credentials.pwd,function(){d(e,function(b){var d=c(b),f=document.getElementById("public-project-list");f.model={projects:d};var g=e.github.getRepo(window.gui.credentials.uid,"beatproducer-projects");g.getBranches().then(function(b){for(var c=[],d=0;d<b.length;d++){var e=b[d].pop();"master"!==e&&c.push({name:e,user:window.gui.credentials.uid})}var f=document.getElementById("private-project-list");f.model={projects:c},a("#gui-loading-progress").fadeOut("fast",function(){document.querySelector("#project-open-dialog").toggle()})})})})},s.prototype.onSaveProjectClick=function(){if(!window.gui.credentials.uid||!window.gui.credentials.pwd)return void(new r).onError({status:401,message:'{"errors":[{"resource": "New Project","message":"No user login"}]}'});if(!window.gui.user)return void window.gui.onNewProjectClick();var b=window.gui.user.branchname;return""===b?void window.gui.onNewProjectClick():void a("#gui-loading-progress").fadeIn("fast",function(){var c=window.gui.user,d="Just improved my cool song. Boom!",e=window.gui.studio.project.configString();c.saveSong(e,d,function(){window.gui.user=c,a("#gui-loading-progress").fadeOut("fast",function(){window.gui.alert("Saved "+b)})})})},s.prototype.onMyWorkspaceClick=function(){return window.gui.credentials.uid&&window.gui.credentials.pwd?void window.open("https://github.com/"+window.gui.credentials.uid+"/beatproducer-projects"):void(new r).onError({status:401,message:'{"errors":[{"resource": "New Project","message":"No user login"}]}'})},s.prototype.onDiscussSongClick=function(){window.currentPublicProjectName?window.open("https://github.com/s-a/beatproducer-projects/issues/new?title="+window.currentPublicProjectName):window.gui.alert("You can only discuss published songs. Open a public song now and give your two cents!")},s.prototype.onNewProjectClick=function(){var b=a.trim(prompt("Please give your new masterpice a unique name.","new-song")).toLowerCase().replace(/ /g,"-");return window.gui.credentials.uid&&window.gui.credentials.pwd?void(""===b?window.gui.alert("Please enter a songname!"):a("#gui-loading-progress").fadeIn("fast",function(){var c=new window.boom.User;c.login(window.gui.credentials.uid,window.gui.credentials.pwd,function(){c.newSong(b,function(){var d="created a new song. yeah!",e=window.gui.studio.project.configString();c.saveSong(e,d,function(){window.gui.user=c,a("#gui-loading-progress").fadeOut("fast",function(){window.gui.alert("Created new song "+b)})})})})})):void(new r).onError({status:401,message:'{"errors":[{"resource": "New Project","message":"No user login"}]}'})},s.prototype.onNewSamplerClick=function(){var a=window.gui,b=a.studio.project._el.state.devices;b.push(new j),a.studio.project._el.setState({devices:b})},s.prototype.onPublishClick=function(){var b=window.gui;if(b.user&&b.user.fork&&b.user.branch){/*var username = window.gui.credentials.uid;
